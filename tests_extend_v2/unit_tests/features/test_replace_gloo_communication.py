...
from tests_extend_v2.commons import initialize_model_parallel as initialize_model_parallel_comm
...
class TestGatherAndScatter(DistributedTest):
    ...
    def test_gather(self, tp_pp):
        ...
        if data_parallel_rank == 0:
            recv_tensors_gloo = [
                torch.empty(send_shape, dtype=torch.float32, device="cpu")
                for _ in range(data_parallel_world_size)
            ]
        else:
        ...
        # HCCL Slice Communication Optimization
        recv_tensors_hccl = [
            torch.empty(send_shape, dtype=torch.float32)
            for _ in range(data_parallel_world_size)
        ]
        _gather_hccl(
            send_tensor,
