"""System test for variable sequence length.
...
class VarSeqLengthModel(nn.Module):

    def __init__(self, input_size):
        super(VarSeqLengthModel, self).__init__()
        args = get_args()
...
def model_provider(pre_process=True, post_process=True):
    ...
    model = VarSeqLengthModel(args.hidden_size)
    model.config = config
...
def forward_step(data_iterator, model):
    ...
    if args.virtual_pipeline_model_parallel_size is not None:
        ...
        vpp_rank = 0
    pp_rank = parallel_state.get_pipeline_model_parallel_rank()
    seq_length = torch.randint(10, 100, (1,)).item()
