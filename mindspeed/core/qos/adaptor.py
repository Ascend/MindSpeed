...
def create_group_qos(
    ...
    if is_a3_version:
        if args.aiqos_enable_roce:
            kwargs['pg_options'].hccl_config = {
                'hccl_sdma_qos': sdma_qos,
                'qos_service_level': roce_qos,
...
def create_hierarchical_groups_qos(rank, ranks, hierarchical_group_sizes, pg_options, parallel_type):
...
def initialize_model_parallel_qos(
    ...
    if mcps._DATA_PARALLEL_GROUP is not None:
    ...
    for ranks in generator_wrapper('dp'):
        ...
        if create_gloo_process_groups:
            group_gloo = create_group(ranks, timeout=timeout, backend="gloo", group_desc='DATA_PARALLEL_GROUP_GLOO')
        else:
            group_gloo = None
        if rank in ranks:
    ...
    if not ((data_parallel_size * context_parallel_size) % num_distributed_optimizer_instances == 0):
        raise RuntimeError('Data parallel size should be divisible by partial DistOpt shard factor')
        ...
        if num_distributed_optimizer_instances > 1:
            for i in range(num_distributed_optimizer_instances):
                ...
                if rank in intra_partial_data_parallel_ranks_with_cp:
                    mcps._INTRA_PARTIAL_DATA_PARALLEL_GROUP_WITH_CP = intra_partial_data_parallel_group_with_cp
            ...
            for i in range(intra_partial_data_parallel_size):
                ...
                if rank in inter_partial_data_parallel_ranks_with_cp:
                    mcps._INTER_PARTIAL_DATA_PARALLEL_GROUP_WITH_CP = inter_partial_data_parallel_group_with_cp
        else:
    ...
    if mcps._CONTEXT_PARALLEL_GROUP is not None:
    ...
    for ranks in generator_wrapper('cp'):
        ...
        if hierarchical_context_parallel_sizes:
            mcps._HIERARCHICAL_CONTEXT_PARALLEL_GROUPS += create_hierarchical_groups_qos(
                rank,
                ranks,
    ...
    if mcps._MODEL_PARALLEL_GROUP is not None:
    ...
    if mcps._TENSOR_MODEL_PARALLEL_GROUP is not None:
    ...
    if mcps._PIPELINE_MODEL_PARALLEL_GROUP is not None:
        raise RuntimeError('pipeline model parallel group is already initialized')
    if mcps._EMBEDDING_GROUP is not None:
        raise RuntimeError('embedding group is already initialized')
    if mcps._POSITION_EMBEDDING_GROUP is not None:
    ...
    if pipeline_model_parallel_comm_backend == 'ucc':
        ...
        os.environ['UCX_TLS'] = os.environ['UCX_TLS'] if "UCX_TLS" in os.environ else 'ib,cuda_copy'
    ...
    if mcps._TENSOR_AND_DATA_PARALLEL_GROUP is not None:
    ...
    if mcps._TENSOR_AND_CONTEXT_PARALLEL_GROUP is not None:
    ...
    if mcps._EXPERT_MODEL_PARALLEL_GROUP is not None:
    ...
    if mcps._EXPERT_TENSOR_PARALLEL_GROUP is not None:
    ...
    if mcps._EXPERT_TENSOR_AND_MODEL_PARALLEL_GROUP is not None:
        raise RuntimeError('Expert tensor + model parallel group is already initialized')
    ...
    if mcps._EXPERT_TENSOR_MODEL_PIPELINE_PARALLEL_GROUP is not None:
    ...
    if mcps._EXPERT_DATA_PARALLEL_GROUP is not None:
        raise RuntimeError('Expert data group is already initialized')
    if mcps._EXPERT_DATA_PARALLEL_GROUP_GLOO is not None:
