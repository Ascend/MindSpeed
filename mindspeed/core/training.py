...
from megatron.training.global_vars import set_args, get_tensorboard_writer, get_wandb_writer, get_one_logger
...
from mindspeed.core.performance.auto_pipeline_perf.autopipeline_perf import (
    autopipelineperf_profiling,
    check_out_of_memory,
    calculate_num_of_activations,
    check_skip_profiling,
)
from mindspeed.core.performance.auto_pipeline_perf.optimpipeline_solver import (
    solve_optimpipeline,
    broadcast_oom_in_ranks,
    broadcast_mbs_in_ranks,
    save_profiling_data,
)
from mindspeed.core.performance.auto_pipeline_perf.schedulepipeline_solver import (
    solve_pipelineschedule,
    broadcast_enable_schedule_in_ranks,
    broadcast_scheduler_in_ranks,
    broadcast_layer_in_ranks,
    all_gather_time,
    average_time_by_rank,
)
from mindspeed.core.memory.auto_pipeline.autopipeline_apply import apply_autopipeline
from mindspeed.core.memory.auto_pipeline.autopipeline_solver import (
    solve_autopipeline,
    broadcast_policy_in_ranks,
    destroy_global_vars,
)
...
def train_decorator(train):
    ...
    def wrapper(*args, **kwargs):
        ...
        is_profile = (
            hasattr(args_, 'profile_npu')
            and args_.profile_npu
        ...
        if is_profile:
            ...
            experimental_config = torch_npu.profiler._ExperimentalConfig(
                aic_metrics=torch_npu.profiler.AiCMetrics.PipeUtilization, profiler_level=profiler_level, l2_cache=False
            )
...
def training_log(
    ...
    for key in loss_dict:
        if not skipped_iter:
            ...
            is_nan = math.isinf(value) or math.isnan(value)
    ...
    if iteration % args.log_interval == 0:
        ...
        torch.distributed.all_reduce(counts_0_tensor, op=torch.distributed.ReduceOp.SUM)
        torch.distributed.all_reduce(counts_1_tensor, op=torch.distributed.ReduceOp.SUM)
                ...
                while mbs_tries < ORIGIN_MBS + 2:
                    ...
                    if mbs_tries <= ORIGIN_MBS and is_oom:
                        raise AssertionError(
                            'A risk of Out of Memory could occur, please reset to a smaller micro batch size.'
                        )
                    if mbs_tries > ORIGIN_MBS and is_oom:
                        break
                if len(global_context) > 0:
                    OPTIMIZED_MBS_LIST, mbs_time = solve_optimpipeline(argument, DATA_PARALLEL_SIZE, global_context)
                PP_SCHEDULE_LIST, pp_schedule_time, OPTIMAL_LAYERS = solve_pipelineschedule(
                    argument, DATA_PARALLEL_SIZE, num_forwards_first_stage, forward_time_dict, backward_time_dict
                )
