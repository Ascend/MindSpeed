...
def parallel_transformer_checkpointed_forward(
    ...
    if self.recompute_method == 'uniform':
        ...
        for layer_idx in range(self.num_layers):
            ...
            should_recompute_layer = recompute_priority < self.recompute_num_layers
            if global_args.reduce_recompute_for_last_chunk:
                is_last_layer = layer_idx == self.num_layers - 1 and mpu.is_pipeline_last_stage()
                should_recompute_layer = should_recompute_layer and not is_last_layer

            if should_recompute_layer and not global_args.swap_attention:
                hidden_states = tensor_parallel.checkpoint(
