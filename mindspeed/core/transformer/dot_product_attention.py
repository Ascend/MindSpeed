...
def dot_product_attention_forward_wrapper(fn):
    ...
    def wrapper(self, query, key, value, attention_mask, attn_mask_type, attention_bias, packed_seq_params):
        if (
            ...
            and getattr(get_args(), 'context_parallel_algo', None) == 'deepseek_v4_cp_algo'
        ):
...
def dot_product_attention_forward(
    ...
    args = get_args()
    if self.config.context_parallel_size > 1 and args.context_parallel_algo == 'deepseek_v4_cp_algo':
        raise NotImplementedError(
            "deepseek_v4_cp_algo requires the DeepSeek V4 compressed-KV CP communication path "
            "and is not supported by standard DotProductAttention."
        )
