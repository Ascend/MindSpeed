...
def compute_dsa_indexer_loss(
    ...
    index_mask = torch.full((b, sq, sk), float("-inf"), dtype=torch.float32, device=causal_mask.device).scatter_(
        -1, topk_indices, 0
    )
    ...
    kl_per_element = attention_scores * (torch.log(attention_scores + 1e-10) - torch.log(index_scores + 1e-10))
...
class MLASelfAttentionAbsorb(MLASelfAttention):
    ...
    def forward(
        ...
        *,
        ...
        # self or cross attn.
        # 0.17 always returns 5 values: (query, key, value, q_compressed, kv_compressed)
            ...
            if inference_context is None or inference_context.is_static_batching():
                with FineGrainedActivationOffloadingInterface.get_context(self.offload_core_attention):
                    if self.config.experimental_attention_variant is None:
                        ...
                        raise ValueError(f"Unsupported attention variant: {self.config.experimental_attention_variant}")
        ...
        if is_tnd:
            ...
            core_attn_out = core_attn_out.view(
                core_attn_out.size(0), core_attn_out.size(1), self.num_attention_heads_per_partition, -1
            )
    ...
    def get_query_key_value_tensors(
        ...
        *,
        ...
        Aligned with Megatron 0.17 convention: always returns 5 values
        ...
        def qkv_up_proj_and_rope_apply(q_compressed, kv_compressed, k_pos_emb, rotary_pos_emb):
            ...
            if args.apply_rope_in_complex:
                q_pos_emb = apply_rope_in_complex(q_pos_emb, rotary_pos_emb, mscale=mscale)
                k_pos_emb = apply_rope_in_complex(k_pos_emb, rotary_pos_emb, mscale=mscale)
            else:
                # shape q_pos_emb [num_tokens, n, qk_pos_emb_head_dim]
