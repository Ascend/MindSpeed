...
def fused_lightning_indexer_kvallgather(
    ...
    if is_tnd:
        ...
        q, weights = [t.view(2, t.shape[0] // 2, *t.shape[1:]) for t in [q, weights]]
...
def fused_npu_sparse_flash_attention_kvallgather(
    ...
    if is_tnd:
        ...
        q, q_rope = [t.view(2, t.shape[0] // 2, *t.shape[1:]) for t in [q, q_rope]]
        # [T, ...] -> [cp*T, ...] (reorder for causal order, no transpose)
        k_ag = gather_and_permute_cp_shard(k, cp_group)
    ...
    *,
    ...
    if is_tnd:
        ...
        query, query_rope, query_index, weights = [
            t.view(2, t.shape[0] // 2, *t.shape[1:]) for t in [query, query_rope, query_index, weights]
        ]
        ...
        softmax_max, softmax_sum = [
            t.view(t.shape[0], 2, t.shape[1] // 2, t.shape[2]).permute(1, 0, 2, 3) for t in [softmax_max, softmax_sum]
        ]
        ...
        # BSND: [s, b, ...] -> [2, s//2, b, ...] -> [2, b, s//2, ...]
        query, query_rope, query_index, weights = [
            t.view(2, t.shape[0] // 2, *t.shape[1:]).transpose(1, 2) for t in [query, query_rope, query_index, weights]
        ]
        ...
        # [b, 1, s, n] -> [2, b, 1, s//2, n]
        softmax_max, softmax_sum = [
            rearrange(t, 'b n2 (c s) n1 -> c b n2 s n1', n2=1, c=2) for t in [softmax_max, softmax_sum]
        ]
