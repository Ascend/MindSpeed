...
def base_moe_init_wrapper(init_func):
    ...
    def base_moe_init(*args, **kwargs):
        ...
        if global_args.moe_tp_extend_ep:
            if global_args.moe_token_dispatcher_type == 'alltoall_seq':  # nosec B105 - dispatcher enum, not a credential
...
def moe_layer_init_wrapper(init_func):
    ...
    def moe_layer_init(*args, **kwargs):
        ...
        if (
            self.config.add_bias_linear and self.config.moe_token_dispatcher_type != 'alltoall'  # nosec B105 - dispatcher enum, not a credential
        ):
        ...
        if (
            hasattr(self.config, 'moe_token_dispatcher_type') and self.config.moe_token_dispatcher_type == 'allgather'  # nosec B105 - dispatcher enum, not a credential
        ):
            ...
            self.recompute_threshold = (
                ...
                * self.moe_adaptive_recompute_activation_scale
...
def moe_adaptive_forward(self, hidden_states: torch.Tensor):
    ...
    moe_adaptive_recompute_activation_bool = self.moe_adaptive_recompute_activation and (
        self.token_dispatcher.all_tokens_per_expert is None
        or torch.max(self.token_dispatcher.all_tokens_per_expert) > threshold
    )
