...
def zerc_preprocess(self, indices: torch.Tensor, probs: torch.Tensor) -> torch.Tensor:
    ...
    self.input_splits = token_send.sum(-1).detach().to(torch.device("cpu"), non_blocking=True).numpy() # [tp_extended_ep_size]
    self.output_splits = tokens_received.sum(-1).to(torch.device("cpu"), non_blocking=True).numpy() # [tp_extended_ep_size]
...
def zerc_alltoall_token_perm1(
    ...
    tokens_per_expert = zerc_preprocess(self, indices, probs)
    ...
    return permutated_local_input_tokens.type(input_dtype), permuted_local_probs, tokens_per_expert
...
def zerc_alltoall_token_perm2(self, global_input_tokens, global_input_token_probs=None):
    ...
    self.nr_token_id_recover = torch.nonzero(self.global_map_info.ravel()).ravel()
    self.probs = torch.index_select(self.global_map_info.ravel(), 0, self.nr_token_id_recover)
    self.nr_token_id_recover = self.nr_token_id_recover % (self.global_map_info.shape[-1])
...
def zerc_alltoall_token_unperm1(
    ...
    if not get_args().moe_unperm2_mem_optim:
        hidden_states = hidden_states * self.probs.unsqueeze(-1)
