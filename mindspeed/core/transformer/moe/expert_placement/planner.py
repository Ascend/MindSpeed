...
def expert_placement_init(self, ep_size):
    ...
    if getattr(global_args, "enable_expert_placement", False):
        self.expert_mapping = torch.arange(
            self.config.num_moe_experts, dtype=torch.int, device=torch.cuda.current_device()
        )
        ...
        self.local_resorted_relative_mapping_indices = torch.zeros(
            self.experts.num_local_experts, dtype=torch.int, device=torch.cuda.current_device()
        )
        self.new_local_expert_sorted_indices = torch.zeros(
            self.experts.num_local_experts, dtype=torch.int, device=torch.cuda.current_device()
        )
...
def predict_expert_load(self, num_tokens_per_expert):
    with torch.no_grad():
        ...
        ema_weight = 0.9
        self.expert_load_prediction = (
            ema_weight * self.expert_load_prediction + (1 - ema_weight) * self.num_tokens_per_expert_origin
        )
...
def gather_expert_load_data_parallel(num_tokens_per_expert):
    ...
    if world_size > 1:
        ...
        torch.distributed._all_gather_base(
            num_tokens_per_expert_overall, num_tokens_per_expert.contiguous(), group=data_parallel_group
        )
            ...
            for layer_index, layer in enumerate(decoder.layers):
                if hasattr(layer, 'mlp') and hasattr(layer.mlp, 'router'):
                    ...
                    if not getattr(args, "enable_expert_placement", False):
                        ...
                        expert_load_per_device_after_placement = (
                            layer.mlp.expert_load_prediction[sorted_expert_mapping_indices]
                            .reshape(-1, num_local_experts)
                            .sum(-1)
                        )
