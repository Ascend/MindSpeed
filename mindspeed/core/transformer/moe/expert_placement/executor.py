...
def expert_weight_and_optimizer_state_placement(args, model, optimizer):
    ...
    if expert_placement_triggered:  # pylint: disable=too-many-nested-blocks
...
def initialize_expert_placement_dispatcher(mlp):
    with torch.no_grad():
        ...
        expert_load = dict(
            zip(range(mlp.config.num_moe_experts), mlp.expert_load_prediction.to(torch.device("cpu")).numpy())
        )
        expert_device_mapping, device_expert_mapping = mlp.expert_placement_optimizer.expert_placement_greedy(
        ...
        device_full_expert_idx = set(range(mlp.experts.num_local_experts))
        ...
        if args.enable_fine_grained_expert_placement:
            ...
            expert_load_per_device_pre = (
                mlp.expert_load_prediction[sorted_expert_mapping_indices]
                .reshape(-1, mlp.experts.num_local_experts)
                .sum(-1)
            )
            expert_load_variation_pre = (
                expert_load_per_device_pre.std() / expert_load_per_device_pre.mean()
            ...
            expert_load_per_device_curr = (
                mlp.expert_load_prediction[sorted_expert_mapping_indices]
                .reshape(-1, mlp.experts.num_local_experts)
                .sum(-1)
            )
            expert_load_variation_curr = (
                expert_load_per_device_curr.std() / expert_load_per_device_curr.mean()
            ...
            if torch.distributed.get_rank() == 0:
                print(
                    f"expert_load_variation_pre {expert_load_variation_pre} \
        ...
        experts_local_indices_mask = torch.logical_and(
            local_expert_indices_offset <= mlp.expert_mapping,
            mlp.expert_mapping < local_expert_indices_offset + mlp.experts.num_local_experts,
        )
        ...
        mlp.input_splits_for_expert_placement = (
            torch.histc(sorted_local_expert_mapping, bins=mlp.ep_world_size, min=0, max=mlp.ep_world_size).cpu().numpy()
        )
...
def expert_weight_placement(mlp):
    ...
    with torch.no_grad():
        ...
        if mlp.config.moe_grouped_gemm:
            group_mlp_expert_params = get_grouped_expert_params(
                mlp.experts, mlp.experts.num_local_experts, tp_size, mlp.experts.config
            )
        ...
        input_paras_buffer = torch.zeros(
            (mlp.experts.num_local_experts, params_size), dtype=params_type, device=torch.cuda.current_device()
        )

        for i in range(mlp.experts.num_local_experts):
            get_expert_param_data(
                group_mlp_expert_params, input_paras_buffer[i], mlp.new_local_expert_sorted_indices[i].item()
            )
        ...
        for idx in range(mlp.experts.num_local_experts):
            set_expert_param_data(
                ...
                output_paras_buffer[mlp.local_resorted_relative_mapping_indices[idx].item()],
                idx,
            )
...
def get_bucket_param_placement_permission(optimizer, bucket):
    ...
    for param in bucket.params_list:
        if (
            param in optimizer.params_module_mlp_map.keys()
    ...
    for buffer in optimizer.buffers:
        for _, bucket in enumerate(buffer.buckets):
            ...
            if reuse_fp32_param:
                ...
                # Must wait for the all-gather before re-viewing/all2all on the same
                # buffer (avoids data race when overlap_param_gather is enabled).
...
def bidirectional_copy_bucket_optimizer_buffer_with_optimizer_mainparam(
    ...
    copy_group_params(
        optimizer.shard_fp32_from_float16_groups, optimizer.model_float16_groups, bucket_buffer_to_mainparam
    )
...
def _dispatch_gather_optimizer_states(optimizer, optbuf_view_items_dp, force_sync: bool = False):
    ...
    if optbuf is not None:
        all_gather_handle = torch.distributed._all_gather_base(
            optbuf,
            optbuf_views[data_parallel_rank],
...
def _get_optimizer_state_buffer_ep_views(optimizer, buffer, bucket, optimizer_state_placement_memory):
    ...
    for _, param in enumerate(bucket.params_list):
        ...
        if ".mlp.router" in optimizer.params_module_mlp_map[param][1]:
            ...
            local_resorted_relative_mapping_indices = optimizer.params_module_mlp_map[param][
                0
            ...
            param_optimizer_data_reshape = param_optimizer_data.view(torch.Size([num_local_experts, numele_per_expert]))
