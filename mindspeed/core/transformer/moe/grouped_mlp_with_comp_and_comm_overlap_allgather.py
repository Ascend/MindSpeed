...
class GroupedMlpWithCompAndCommOverlapAllGather(torch.autograd.Function):
    @staticmethod
    def forward(ctx, inputs, weights1, weights2, activation_func, group_list):
    ...
    def backward(ctx, *grad_outs):
        ...
        _, ag_inputs_tp_ep, ag_handle = async_all_gather(ag_inputs_tp, get_expert_model_parallel_group() if get_args().n_shared_experts else get_tensor_and_expert_parallel_group())

        # grad of mm1-inputs
        weights1 = rearrange(weights1, 'n h f -> n f h')
        mm1_inputs_grad = GMMFunction.builder.load().npu_gmm([mm1_outs_grad], [weights1], [], group_list.tolist(), 0, group_list_data_type)[0]
        # token 反重排的反向
        ...
        mm1_weights_grad = GMMFunction.builder.load().npu_gmm([mm1_inputs.t()], [mm1_outs_grad], [], group_list.tolist(), 2, group_list_data_type)[0]
...
def grouped_mlp_with_comp_and_comm_overlap_allgather(inputs, weights1, weights2, activation_func, group_list):
    return GroupedMlpWithCompAndCommOverlapAllGather.apply(inputs, weights1, weights2, activation_func, group_list)
