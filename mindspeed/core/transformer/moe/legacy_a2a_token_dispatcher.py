...
def allgather_token_permutation(self, hidden_states: torch.Tensor, max_prob: torch.Tensor, routing_map: torch.Tensor):
    ...
    if self.tp_size > 1 or self.ep_size > 1:
        # [S*B/TP, H] -> [S*B, H]
        with torch.no_grad():
            routing_map, gi_handle = (
                routing_map
                if isinstance(routing_map, tuple)
                else gather_from_sequence_parallel_region_to_moe_async(routing_map)
            )
    ...
    gi_handle.wait()  # pylint: disable=possibly-used-before-assignment
    self.local_map = routing_map[:, self.local_expert_indices[0] : self.local_expert_indices[-1] + 1].contiguous()

    gp_handle.wait()  # pylint: disable=possibly-used-before-assignment
    self.local_probs = max_prob[:, self.local_expert_indices[0] : self.local_expert_indices[-1] + 1].contiguous()

    tokens_per_expert = self.local_map.sum(dim=0).long().cpu()
    ghs_handle.wait()  # pylint: disable=possibly-used-before-assignment
...
class NewIndePut(torch.autograd.Function):
    @staticmethod
    def forward(self, tensor, map_, value_):  # pylint: disable=bad-staticmethod-argument
        self.map_ = map_
        ori_dtype = None
...
def allgather_token_unpermutation(
    self,
    ...
    if cann_version_check:
        ...
        unpermuted_local_hidden = unpermute(hidden_states, self.reversed_local_input_permutation_mapping)
    ...
    if (self.config.tensor_model_parallel_size > 1) or (self.config.expert_model_parallel_size > 1):
        ...
        if cann_version_check:
            ...
            unpermuted_global_hidden = NewIndePut.apply(
            ...
            unpermuted_global_hidden = moe_scatter.apply(  # noqa: F821  # pylint: disable=undefined-variable
        ...
        if self.add_bias:
            ...
            if cann_version_check:
                unpermuted_global_bias.index_put_(
                    (self.global_local_map,),
...
def preprocess(self, routing_map: torch.Tensor) -> torch.Tensor:
    ...
    if self.ep_size > 1 or self.tp_size > 1:
        ...
        num_global_tokens_per_expert = (
            tensor_parallel.gather_from_sequence_parallel_region(num_local_tokens_per_expert, group=self.tp_ep_group)
        ...
        # from other EP rank.
        self.output_splits = num_global_tokens_per_rank[self.tp_rank].to(torch.device("cpu"), non_blocking=True).numpy()
...
def allgather_token_permutation_new(self, global_routing_map_tuple, global_probs_tuple, global_hidden_states_tuple):
    ...
    self.local_probs = global_probs[:, self.local_expert_indices[0] : self.local_expert_indices[-1] + 1].contiguous()
...
def allgather_token_unpermutation_new(
    self,
    hidden_states: torch.Tensor,
    bias: torch.Tensor = None,
...
def alltoall_token_permutation_new(
        ...
        if shared_expert_gate is not None:
            with torch.enable_grad():
                ...
                share_experts_output = (
                    torch.nn.functional.sigmoid(shared_expert_gate(hidden_states)) * share_experts_output
                )
    else:
    ...
    # token 重排2 input
    (global_input_tokens), global_input_tokens_detach = forward_func(alltoall_token_permutation2, global_input_tokens)  # pylint: disable=unbalanced-tuple-unpacking
...
def alltoall_token_unpermutation_new(self, hidden_states, bias, save_tensors):
    ...
    hidden_states, unpermute1_input_detach = forward_func(alltoall_token_unpermutation1, hidden_states)  # pylint: disable=unbalanced-tuple-unpacking
    ...
    output, unpermute2_input_detach = forward_func(alltoall_token_unpermutation2, permutated_local_input_tokens)  # pylint: disable=unbalanced-tuple-unpacking
