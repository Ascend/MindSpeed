...
def transformer_layer_forward_dense_backward_balanced_moe_overlaping(
    ...
    checkpoint_context = torch.no_grad() if checkpoint else nullcontext()

    def _maybe_wait(handle):
    ...
    (
        bwd_cold_local_hot_input,
        bwd_remote_hot_input,
        bwd_probs,
    ...
    if args.moe_zero_memory == 'level0':
        with torch.no_grad():
            ...
            bwd_perm1_out, bwd_perm1_remote_hot_out = recomp_token_permutation1(
                bwd_input_before_perm1, bwd_sorted_routing_map, bwd_sumnum_cold_local_hot_tokens
            )

    _groupedmlp_hot_expert_params_broadcast(
        bwd_layer_graph.layer.mlp.experts,
        bwd_layer_graph.hot_experts_list,
        bwd_layer_graph.layer.mlp.hot_experts,
        bwd_layer_graph.params,
    )
    ...
    _maybe_wait(bwd_unperm_a2a_handle)
    bwd_unperm_a2a_handle = None
    run_graph_backward(bwd_layer_graph.unperm1_graph, unperm1_out_grad)  # pylint: disable=possibly-used-before-assignment
    ...
    if hot_expert_broadcast_handles:
    ...
    WeightGradStore.start_decouple()
    run_graph_backward(bwd_layer_graph.grouped_mlp_graph, keep_grad=True)
    WeightGradStore.end_decouple()
    ...
    if args.moe_zero_memory == 'level0':
        with torch.no_grad():
            # Recompute perm_a2a_out for dw computation (same as balanced-moe<->balanced-moe path)
            (bwd_perm_a2a_out, bwd_recomp_perm_a2a_handle), _ = bwd_dispatcher.async_dispatch_comm(
                bwd_perm1_out,  # pylint: disable=possibly-used-before-assignment
                output_splits=bwd_layer_graph.output_splits,
                input_splits=bwd_layer_graph.input_splits,
                output_splits_tp=bwd_layer_graph.output_splits_tp,
            )
            _maybe_wait(bwd_recomp_perm_a2a_handle)
            bwd_recomp_perm_a2a_handle = None
            recompute_fc1_input, _ = bwd_dispatcher.token_permute2(
                bwd_perm_a2a_out,  # pylint: disable=possibly-used-before-assignment
                None,
                bwd_num_global_tokens_per_local_expert_cpu,  # pylint: disable=possibly-used-before-assignment
            )
    ...
    _maybe_wait(bwd_perm_a2a_handle)
    bwd_perm_a2a_handle = None

    if bwd_prob_handle:
        _maybe_wait(bwd_prob_handle)
    if use_shared_experts:
    ...
    perm1_remote_hot_grad = (
        bwd_layer_graph.perm1_graph[1][0].grad if bwd_layer_graph.perm1_graph[1][0] is not None else None
    )
    perm1_remote_prob_grad = (
        bwd_layer_graph.perm1_graph[1][1].grad if bwd_layer_graph.perm1_graph[1][1] is not None else None
    )
    run_graph_backward(
...
def transformer_layer_forward_balanced_moe_backward_dense_overlaping(
    ...
    with checkpoint_context:
        ...
        if use_shared_experts:
            fwd_shared_experts.pre_forward_comm(detached_mlp_input, wait_event=None)
        ...
        ) = fwd_dispatcher.token_permute1(detached_mlp_input, probs_detached, routing_map)
    ...
    with checkpoint_context:
        (perm_a2a_out, perm_a2a_handle), (perm_prob_a2a_out, perm_prob_a2a_handle) = fwd_dispatcher.async_dispatch_comm(
            perm1_local_out, perm1_local_probs, wait_event=None
        )
    ...
    with checkpoint_context:
        ...
        detached_dispached_cold_local_hot_input_probs = detach_tensor(
            dispached_cold_local_hot_input_probs, checkpoint_forward=checkpoint
        )
        ...
        (
            expert_cold_local_hot_output,
            cold_local_act_ckpt_manager,
            expert_remote_hot_output,
            remote_hot_act_ckpt_manager,
        ...
        if args.moe_zero_memory == 'level0':
            ...
            recompute_needed_tensors = [
                dispached_cold_local_hot_input,
                dispached_remote_hot_input,
                probs,
    ...
    unperm_a2a_handle.wait()  # pylint: disable=used-before-assignment
    ...
    with checkpoint_context:
        detached_unperm_a2a_cold_local_hot_out = detach_tensor(unperm_a2a_out)  # pylint: disable=possibly-used-before-assignment
    ...
    saved_tensors = (
        ...
        (
            ...
            (detached_dispached_remote_hot_input, detached_remote_hot_input_probs),
        ),
        (None, (detached_perm_a2a_out, detached_perm_prob_a2a_out)),
        (
            (dispached_cold_local_hot_input, dispached_cold_local_hot_input_probs),
            (detached_dispached_cold_local_hot_input, detached_dispached_cold_local_hot_input_probs),
        ),
        (
            (expert_cold_local_hot_output, expert_remote_hot_output),
            (detached_expert_expert_cold_local_hot_output, detached_expert_remote_hot_output),
        ),
        (unperm1_local_out, None),
        (None, detached_unperm_a2a_cold_local_hot_out),
        (output, None),
        (share_experts_graph, detached_shared_expert_output),  # pylint: disable=possibly-used-before-assignment
        detached_layer_input,
    )
