...
def get_parallel_num(m, k, n, default_parallel_num=None):
    parallel_num = default_parallel_num if default_parallel_num is not None else min_comm_config.parallel_num
    shape_str = str([m, k, n])
...
class COCParallel:
    """
    when not use fused ops, this class control compute first or communicate first
    """
    ...
    def __init__(
        ...
        self.split_num = parallel_num if parallel_num is not None else min_comm_config.parallel_num
