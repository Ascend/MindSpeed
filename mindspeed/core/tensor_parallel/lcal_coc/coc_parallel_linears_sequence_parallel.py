...
from .coc_utils import reshape_to_2D, async_gather_along_first_dim, is_grad_needed, get_parallel_num
...
class COCColumnSeqParallelFunction(torch.autograd.Function):
    ...
    def forward(ctx, input_, weight, bias):
        ...
        parallel_num = get_parallel_num(
            m=reduce(lambda x, y: x * y, input_.shape[:-1]) * min_comm_config.tp_world_size,
        ...
        if min_comm_config.matmul_soc_friendly_enabled:
            input_, trans_weight = get_aligned_mm_inputs(
                input_, trans_weight, sp_coef=min_comm_config.tp_world_size, parallel_num=parallel_num
            )
        ...
        coc_parallel = COCParallel(
            input_,
            CommunicationType.ALL_GATHER,
            compute_fcn,
            compute_first=False,
        ...
        if not min_comm_config.all_gather_recomputation_enabled:
            total_input = shuffle_as_coc_reduce_scatter(
                coc_parallel.comm_output, min_comm_config.tp_world_size, parallel_num
            )
    ...
    def backward(ctx, grad_output):
        ...
        if is_grad_weight_needed:
            if min_comm_config.all_gather_recomputation_enabled:
                total_input_work, total_input = async_gather_along_first_dim(
                    input_, min_comm_config.tp_group, min_comm_config.tp_world_size
                )
            else:
        ...
        sub_grad_input_work = torch.distributed._reduce_scatter_base(
            sub_grad_input, grad_input, group=min_comm_config.tp_group, async_op=True
        )
...
class COCRowSeqParallelFunction(torch.autograd.Function):
    ...
    def forward(ctx, input_, weight, bias):
        ...
        parallel_num = get_parallel_num(
            m=reduce(lambda x, y: x * y, input_.shape[:-1]), k=trans_weight.shape[0], n=trans_weight.shape[1]
        )
        if parallel_num == 1:
        ...
        coc_reduce_scatter = COCParallel(
            input_,
            CommunicationType.REDUCE_SCATTER,
            compute_fcn,
            compute_first=True,
            weight_shape_list=list(trans_weight.shape),
            parallel_num=parallel_num,
    ...
    def backward(ctx, grad_output):
        ...
        if min_comm_config.matmul_soc_friendly_enabled:
            grad_output, weight = get_aligned_mm_inputs(
                grad_output, weight, sp_coef=min_comm_config.tp_world_size, parallel_num=parallel_num
            )
        ...
        coc_all_gather = COCParallel(
            grad_output,
            CommunicationType.ALL_GATHER,
            compute_fcn,
            compute_first=False,
            weight_shape_list=list(weight.shape),
            parallel_num=parallel_num,
        )
