...
def broadcast_dynamic(item):
    if item is not None:
        ...
        item_len = torch.empty(
            (), dtype=torch.int64, device=torch.cuda.current_device()
        )
        _broadcast(item_len)
        item = torch.empty(
            [item_len.item()], dtype=torch.int64, device=torch.cuda.current_device()
        )
        _broadcast(item)
...
def generate_rearrange_idx_tensor(tp_y_cp_size):
    ...
    if _REARRANGE_IDX_TENSOR is None:
        ...
        _REARRANGE_IDX_TENSOR = torch.tensor(
            rearrange_index, device="cpu", pin_memory=True
        ).to(device="npu", non_blocking=True)
...
def batch_index(seq1d, seq_len):
    ...
    seq_batch = [seq1d[start:end] for start, end in zip(indexes, indexes[1:])]
...
def get_batch_on_this_cp_rank(batch, mtp_on_this_rank: bool = False):
    ...
    tp_y_cp_size = (
        TensorParallelYUnionCP().get_parallel_group_world_size()
        if args.tp_2d
    ...
    if args.reset_attention_mask and args.attention_mask_type == "causal":
        if args.context_parallel_algo == "ulysses_cp_algo":
            ...
            batch = _get_batch_on_this_cp_rank_in_megatron_cp_eod_padding(
                batch, get_actual_seq_len()
            )
...
def _get_batch_on_this_cp_rank_in_megatron_cp_eod_padding(batch, actual_seq_len):
    if actual_seq_len is None or actual_seq_len.numel() == 0:
        raise AssertionError(
            "actual_seq_len must be provided for EOD load-balanced context parallelism."
        )
    ...
    def get_index(actual_seq_len_cpu, cp_rank, cp_size):
        ...
        if torch.any(seq_lens <= 0):
            raise AssertionError(
                "Each EOD subsequence must have a positive sequence length."
            )
        if torch.any(seq_lens % (2 * cp_size) != 0):
            raise AssertionError(
                "Each EOD subsequence length must be divisible by 2 * context_parallel_size."
            )
        chunk_sizes = seq_lens // (2 * cp_size)
    ...
    for key, val in batch.items():
        ...
        if val is not None:
            ...
            tail_start = seq_dim + 1
            bsz = val.shape[0]
            val = val.view(-1, *val.shape[tail_start:])
            val = val.index_select(0, index)
            val = val.view(bsz, -1, *val.shape[tail_start:])
...
def _get_batch_on_this_cp_rank_in_megatron_cp_general(batch):
    ...
    if attention_mask is not None:
        if len(attention_mask.shape) != 2:
            raise AssertionError(
                "The fusion attention operator currently only support 2D attention mask."
            )
        seq_dim = 0
        mask_row = attention_mask.chunk(cp_size, dim=seq_dim)[cp_rank].contiguous()
        if get_args().attention_mask_on_cpu:
            mask_list = [
                m.contiguous().npu(non_blocking=True)
                for m in mask_row.chunk(cp_size, dim=1)
            ]
        else:
...
def _get_batch_on_this_cp_rank_in_megatron_cp(batch):
    ...
    for key, val in batch.items():
        ...
        if val is not None:
            seq_dim = 1 if key != "attention_mask" else 2
            tail_start = seq_dim + 1
            collapsed_tail_start = seq_dim + 2
            val = val.view(
                *val.shape[0:seq_dim],
                2 * cp_size,
            ...
            val = val.view(*val.shape[0:seq_dim], -1, *val.shape[collapsed_tail_start:])
            batch[key] = val
...
def _get_batch_on_this_cp_rank_in_hybrid_cp_general(batch):
    ...
    if attention_mask is not None:
        ...
        if get_args().attention_mask_on_cpu:
            mask_list = [
                m.contiguous().npu(non_blocking=True)
                for m in mask_row.chunk(r_size, dim=1)
            ]
        else:
...
def _get_batch_on_this_cp_rank_in_adaptive_cp(batch):
    ...
    if args.adaptive_cp_manually_set_mask_list:
        ...
        generate_adaptive_cp_mask_list_by_user(
            remapped_seq_order, scheduling, cp_rank, cp_size
        )
        ...
        remapped_seq_order, scheduling = adaptive_cp_ops.get_adaptive_cp_info(
            attention_mask, cp_size
        )
        mask_list = adaptive_cp_ops.get_mask_list(
            attention_mask, scheduling, remapped_seq_order, cp_rank, cp_size
        )
    ...
    for key, val in batch.items():
        if key != "attention_mask" and val is not None:
            ...
            order_start = cp_rank * per
            order_end = (cp_rank + 1) * per
            index = torch.tensor(
                remapped_seq_order[order_start:order_end],
                device=val.device,
                dtype=torch.int,
            )
            val = val.index_select(seq_dim, index)
...
def _get_batch_on_this_cp_rank_in_hybrid_adaptive_cp(batch):
    ...
    if args.adaptive_cp_manually_set_mask_list:
        ...
        generate_adaptive_cp_mask_list_by_user(
            remapped_seq_order, scheduling, adap_rank, adap_size
        )
        mask_list = get_adaptive_cp_mask_list_by_user()
    else:
        if attention_mask is None:
        ...
        adaptive_cp_ops = AdaptiveCpOps()
        remapped_seq_order, scheduling = adaptive_cp_ops.get_adaptive_cp_info(
            attention_mask, adap_size
        )
        mask_list = adaptive_cp_ops.get_mask_list(
            attention_mask, scheduling, remapped_seq_order, adap_rank, adap_size
        )
    ...
    for key, val in batch.items():
        if key != "attention_mask" and val is not None:
            ...
            order_start = which_per * per
            order_end = (which_per + 1) * per
            index = torch.tensor(
                remapped_seq_order[order_start:order_end],
                device=val.device,
            )
    ...
    if mpu.get_tensor_model_parallel_rank() == 0:
        ...
        if args.reset_attention_mask:
            ...
            if (
                ...
                and args.context_parallel_algo
                in ["megatron_cp_algo", "kvallgather_cp_algo"]
            ):
                ...
                actual_seq_len = actual_seq_len // get_ring_degree()
