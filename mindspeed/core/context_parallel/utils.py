...
def unflatten_softmax(x, sub_seq_len):
    ...
    section_len = [s * orig_shape[1] for s in sub_seq_len]
    splits = x.view(-1, orig_shape[-1]).split(section_len, dim=0)
    merged = [item.view(-1, orig_shape[1], orig_shape[-1]).transpose(0, 1).view(-1, orig_shape[-1]) for item in splits]
    merged = torch.cat(merged, dim=0)
    return merged.view(*orig_shape)


def forward_update_without_fused(
    prev_attn_out,
    prev_softmax_max,
    prev_softmax_sum,
    cur_attn_out,
...
class RingP2P:
    ...
    def async_send_recv(self, orig_send_tensor, orig_recv_tensor, shapes=None):
        ...
        # list-based K/V 传输（enable_mla 分支）
        # 此分支为 MindSpeed 自实现 Ring CP 的 MLA 适配，用于处理 MLA 中 K/V 维度不同
        # （qk_head_dim vs v_head_dim）无法直接 cat(K, V) 传输的问题。
        # 适用场景：TE 模式 + EOD/Unaligned + MLA + Ring CP。
        # TE 模式 + 纯 Ring CP（无独有组合）已下线，走 TENPU 原生 RingCPStrategy，
        # 不经过此代码路径。详见 rfc_cp_adaptation_reduction.md §3.3.5。
...
def forward_update(
    ...
    if hasattr(_args, 'use_fused_ring_attention_update') and _args.use_fused_ring_attention_update:
        ...
        return npu_ring_attention_update(
            prev_attn_out,
            prev_softmax_max,
            prev_softmax_sum,
            cur_attn_out,
...
def tnd_out_update(
    q_block_id, kv_block_id, cur_attn_outs, global_attn_outs, q_index, softmax_indices, cur_sub_out_seq_len
):
    ...
    if q_block_id == kv_block_id:
        ...
        attn_out_updated, softmax_max_updated, softmax_sum_updated = forward_update(
            ...
            cur_softmax_max,
            cur_softmax_sum,
            actual_seq_qlen=cur_sub_out_seq_len,
            layout=layout,
        )
        ...
        attn_out_updated, softmax_max_updated, softmax_sum_updated = forward_update(
            torch.index_select(attn_out, 0, q_index),
            prev_softmax_max,
            prev_softmax_sum,
            cur_attn_out,
            cur_softmax_max,
        ...
        softmax_max = (
            softmax_max.view(-1, 8).index_copy(0, softmax_indices, softmax_max_updated.view(-1, 8)).view(-1, n, 8)
        )
        softmax_sum = (
            softmax_sum.view(-1, 8).index_copy(0, softmax_indices, softmax_sum_updated.view(-1, 8)).view(-1, n, 8)
        )
...
def general_out_update(q_block_id, kv_block_id, cur_attn_outs, global_attn_outs):
    ...
    if q_block_id == kv_block_id:
        ...
        attn_out_updated, softmax_max_updated, softmax_sum_updated = forward_update(
            attn_out, softmax_max, softmax_sum, cur_attn_out, cur_softmax_max, cur_softmax_sum, layout=layout
        )
        attn_out, softmax_max, softmax_sum = attn_out_updated, softmax_max_updated, softmax_sum_updated
