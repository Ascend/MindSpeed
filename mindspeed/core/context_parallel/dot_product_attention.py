...
class CPDotProductAttentionImpl:
    ...
    def forward(
        ...
        if (
            getattr(self.config, 'context_parallel_size', 1) > 1
            and getattr(self.config, 'context_parallel_algo', None) == 'deepseek_v4_cp_algo'
        ):
            raise NotImplementedError(
                "deepseek_v4_cp_algo requires the DeepSeek V4 compressed-KV CP communication path "
                "and is not supported by standard DotProductAttention."
            )
                ...
                if getattr(self.config, 'reset_attention_mask', False):
                    if self.config.attention_mask_type == 'general':
                        ...
                        if not (
                            ...
                            or self.config.context_parallel_algo == 'ulysses_cp_algo'
                        ):
        ...
        is_ulysses_algo = (
            self.config.context_parallel_size > 1
            and getattr(self.config, 'context_parallel_algo', None) == 'ulysses_cp_algo'
        )
        ...
        if packed_seq_params is not None and not is_ulysses_algo:
            # TND
            _, n_head, _ = query.shape
        else:
            _, bsz, n_head, head_dim = query.shape
        ...
        if (
            self.config.context_parallel_size > 1
            and self.config.context_parallel_algo == "ulysses_cp_algo"
            and self.config.context_parallel_kv_cache_policy
        ):
            self.ulysses_comm_para['cache_policy'] = get_cache_policy(
                self.layer_number,
                self.config.context_parallel_kv_cache_policy,
                self.config.context_parallel_cache_interval,
            )
            ...
            return output
        if tp_y_cp_sz > 1 and self.config.context_parallel_algo in [
            'megatron_cp_algo',
            'hybrid_cp_algo',
            'adaptive_cp_algo',
            ...
            if self.config.context_parallel_algo in ['megatron_cp_algo', 'hybrid_cp_algo']:
                is_general_eod = (getattr(self.config, 'attention_mask_type', None) == 'general') and (
                    packed_seq_params is not None
                )
                if is_general_eod:
                    query, key, value = [
                        rearrange(x, '(b s) n d -> s b (n d)', b=self.config.micro_batch_size)
            ...
            if packed_seq_params is not None and is_ulysses_algo:
                # These dimensions are initialized in the matching non-TND branch above.
                query = rearrange(
                    query,
                    's b (h d) -> (b s) h d',
                    d=head_dim,  # pylint: disable=possibly-used-before-assignment
                )
                key = rearrange(
                    key,
                    's b (h d) -> (b s) h d',
                    d=head_dim_k,  # pylint: disable=possibly-used-before-assignment
                )
                value = rearrange(
                    value,
                    's b (h d) -> (b s) h d',
                    d=head_dim_v,  # pylint: disable=possibly-used-before-assignment
                )
                shape_order = 'TND'
            ...
            if packed_seq_params is not None and is_ulysses_algo:
                output = rearrange(
                    output,
                    '(b s) h d -> s b (h d)',
                    b=bsz,  # pylint: disable=possibly-used-before-assignment
                )
                shape_order = 'TND'
