...
def flash_attn_p2p_communicate_o(scheduling_info, send_o_dst, recv_o_src, cp_group, rank):
    ...
    for recv_src in scheduling_info.recv_o_src:
        recv_op = torch.distributed.P2POp(torch.distributed.irecv, recv_o_src, recv_src, group=cp_group,
                                          tag=100000 + rank)
        send_recv_ops.append(recv_op)
        is_recv_o = True
    if scheduling_info.send_o_dst > -1:
        send_dst = scheduling_info.send_o_dst
        send_op = torch.distributed.P2POp(torch.distributed.isend, send_o_dst, send_dst, group=cp_group,
                                          tag=100000 + send_dst)
        send_recv_ops.append(send_op)
    send_recv_ops_o = []
    if len(send_recv_ops) > 0:
