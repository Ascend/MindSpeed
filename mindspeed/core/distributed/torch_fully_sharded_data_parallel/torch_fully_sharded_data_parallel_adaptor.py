        ...
        if module_names:
            try:
                ...
                raise ModuleNotFoundError(
                    f"Module {module_names} Not Found, \
...
def get_fsdp2_mixed_precision_policy(fsdp2_config: dict):
    ...
    fsdp2_config['mp_policy'] = MixedPrecisionPolicy(
        param_dtype=mp_policy_param_dtype,
...
def _create_device_mesh(sharding_size: Optional[int], process_group: ProcessGroup) -> DeviceMesh:
    ...
    group_global_ranks = torch.tensor(get_process_group_ranks(process_group), device="cpu", dtype=torch.int)
...
def torch_fully_sharded_data_parallel_init(
    ...
    assert HAVE_FSDP, 'TorchFullyShardedDataParallel requires PyTorch >= 2.4.0 with FSDP 2 support.'
    ...
    def save_custom_attrs(module):
        ...
        for name, param in module.named_parameters():
            ...
            custom_attrs[name] = dict(attrs.items())
        return custom_attrs
    ...
    sub_modules_to_wrap = (
        sub_modules_to_wrap if fsdp2_config.sub_modules_to_wrap is None else fsdp2_config.sub_modules_to_wrap
    )
        ...
        if args.init_model_with_meta_device:
            ...
            if len(module_states) > 0:
                torch.distributed._broadcast_coalesced(
                    self.device_mesh.get_group(), module_states, 250 * 1024 * 1024, 0
                )
    ...
    if 0 < num_to_forward_prefetch < config.num_layers:
