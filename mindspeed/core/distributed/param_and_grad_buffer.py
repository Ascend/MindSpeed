...
from megatron.core.distributed.param_and_grad_buffer import Bucket, shard_buffer
from mindspeed.optimizer.adamw import ScaleMeta
...
def param_and_grad_buffer_init_wrapper(init_func):
    ...
    def param_and_grad_buffer_init_func(
        ...
        if args.quant_grads:
            bucket_params = set()
            cur_bucket_id = 0
            scaling_grads = [[]] * len(self.buckets)
            ...
            for idx, param in enumerate(params[::-1]):
                ...
                if bucket_id != cur_bucket_id:
                    setattr(self.buckets[cur_bucket_id], "scaling_grads", scaling_grads[cur_bucket_id])
                ...
                scale_meta = ScaleMeta("fp16", param.main_grad, param.main_grad.numel())
                param.main_grad.meta = scale_meta
                param.main_grad.meta.scale = scales[idx:idx + 1]

                scaling_grads[cur_bucket_id].append(param.main_grad)
                scale_end_index += 1
