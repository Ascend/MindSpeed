...
def build_layers_impl(
    ...
    # Resolve through the module so pipeline-layout patches registered after
    # this module was imported are honored as well.
    layer_offset = transformer_layer.get_transformer_layer_offset(
        transformer.config,
        transformer.vp_stage,
        get_pg_rank(transformer.pg_collection.pp),
    )
    ...
    def build_layer(layer_spec, layer_number):
        ...
        return build_module(
            ...
            vp_stage=transformer.vp_stage,
        )

    transformer.layers = torch.nn.ModuleList(
        [build_layer(layer_spec, i + 1) for i, layer_spec in enumerate(transformer.submodules.layer_specs)]
    )

    if transformer.submodules.layer_norm and transformer.post_process and transformer.post_layer_norm:
