...
from mindspeed.core.memory.swap_attention.prefetch import (
...
class AdaptiveRecomputeSwapReg:
    ...
    def register_recursive_apply_prefetch(self, config, models, ctx, prefetch_recompute_group, prefetch_args):
            ...
            for model in models:
                ...
                if any(filter(None, prefetch_layer)):
                    ...
                    self.register_recursive_apply_prefetch(
                        config, model, self.get_list_layers_context(ctx, idx), prefetch_recompute_group, prefetch_args
                    )
                idx += 1
            return
        ...
        if hook_list == prefetch_list and prefetch_list != ['']:
            if "name" in ctx and ctx["name"] in swap_modules and get_layer_id(ctx["prefix_name"]) in prefetch_list:
        ...
        for name, module in models.named_children():
            ...
            prefetch_recompute_group = [prefetch_layer, hook_layer, recompute_layer]
            self.register_recursive_apply_prefetch(
                config, module, ctx['layers'][idx], prefetch_recompute_group, prefetch_args
            )
            idx += 1
