...
def reuse_buffer_single(self):
    ...
    for buffer in self.buffers:
        ...
        shard_res_and_buffer_model_param = torch.zeros(
            buffer_numel * 2, dtype=torch.bfloat16, device=buffer.param_data.device
        )
        shard_main_param_int32_view_buffer = torch.empty(
            buffer_numel, dtype=torch.int32, device=buffer.param_data.device
        )
        ...
        self.model_param_bucket_and_shard_main_param_int32_view_map[shard_res_and_buffer_model_param] = (
            shard_main_param_int32_view_buffer
        )
    for model_fp16_params_this_group, shard_fp32_from_float16_group in zip(
        ...
        for i, (model_param, shard_fp32_main_param) in enumerate(
            ...
            reuse_data_ptr(
                shard_fp32_from_float16_group[i], self.shard_main_param_res_buffers[gbuf_index], data_start_index
            )
            old_param_data = model_param.data
            model_param.data = self.shard_main_param_res_buffers[gbuf_index][
                data_start_index + data_end_index : 2 * data_end_index
            ].view(old_param_data.shape)
...
def reuse_buffer_dis(self, data_parallel_world_size):
    ...
    for buffer in self.buffers:
        ...
        for bucket in buffer.buckets:
            ...
            shard_main_param_int32_view_bucket = torch.empty(
                param_data_dp_numel, dtype=torch.int32, device=bucket.param_data.device
            )
            reuse_data_ptr(
                shard_main_param_int32_view_bucket,
                buffer.param_data,
                (bucket_res_numel * data_parallel_world_size) // 2
                + max(0, data_parallel_rank - 1) * param_data_dp_numel // 2,
            )
            ...
            self.model_param_bucket_and_shard_main_param_int32_view_map[bucket.param_data] = (
                shard_main_param_int32_view_bucket
            )
    ...
    for model_fp16_params_this_group, shard_fp32_from_float16_group in zip(
        ...
        for i, (model_param, shard_fp32_main_param) in enumerate(
            ...
            shard_fp32_param_bucket_offset = (
                world_range.start
                if data_parallel_rank == 0
                else world_range.start - model_param_bucket_numel_per_dp * (1 + data_parallel_rank) // 2
            )
        ...
        if self.reuse_fp32_param:
            ...
            for float16_params_this_group, fp32_from_float16_group in zip(
                ...
                for i, (_, fp32_from_fp16_param) in enumerate(zip(float16_params_this_group, fp32_from_float16_group)):
                    ...
                    self.convert_func.init_and_reuse_storage_of_tensors(
            ...
            if args.npu_deterministic:
                self.fp16_tensor_convert_to_fp32_tensor = types.MethodType(
                    self.convert_func.fp16_tensor_convert_to_fp32_tensor_deterministic, self
                )
                self.fp32_tensor_convert_to_fp16_tensor = types.MethodType(
                    self.convert_func.fp32_tensor_convert_to_fp16_tensor_deterministic, self
                )
            else:
                self.fp16_tensor_convert_to_fp32_tensor = types.MethodType(
                    self.convert_func.fp16_tensor_convert_to_fp32_tensor, self
                )
                self.fp32_tensor_convert_to_fp16_tensor = types.MethodType(
                    self.convert_func.fp32_tensor_convert_to_fp16_tensor, self
                )
