...
class SwapTensors:
    ...
    def __init__(self, tensors):
        ...
        self.storage_sizes: List[int] = [t.untyped_storage().nbytes() for t in self.tensors]
        self.is_slice_tensors: List[bool] = [
            t.untyped_storage().nbytes() != t.numel() * t.element_size() for t in self.tensors
        ]
        self._stat = "device"
    ...
    def add_tensor(self, tensor: torch.Tensor):
        ...
        self.storage_sizes.append(tensor.untyped_storage().nbytes())
        self.is_slice_tensors.append(tensor.untyped_storage().nbytes() != tensor.numel() * tensor.element_size())
            ...
            with torch_npu.npu.stream(stream):
                ...
                for i, tensor in enumerate(self.tensors):
                    ...
                    if self.is_slice_tensors[i]:
                        ...
                        self.tensor_cpus[i].untyped_storage().copy_(tensor.untyped_storage(), non_blocking=True)
            ...
            with torch_npu.npu.stream(stream):
                ...
                for i, tensor in enumerate(self.tensors):
                    if self.tensor_cpus[i] is None:
                        continue
                    tensor.untyped_storage().resize_(self.storage_sizes[i])
                    if self.is_slice_tensors[i]:
                        tensor.copy_(self.tensor_cpus[i], non_blocking=True)
                    else:
                        tensor.untyped_storage().copy_(self.tensor_cpus[i].untyped_storage(), non_blocking=True)
...
class SwapLayerInputManager:
    ...
    def backward_hook(self, _):
        self._ensure_streams()
        if self.layer_idx + 1 < self.num_layers and self.batch_stack and self.batch_stack[0].stat == 'h2d':
