...
def validate_args_wrapper(validate_args):
    ...
    def wrapper(args, defaults=None):
        ...
        if args.moe_alltoall_overlap_comm and not args.moe_token_dispatcher_type == 'alltoall_seq':  # nosec B105
            raise AssertionError(
                '`--moe-alltoall-overlap-comm` only support with `--moe-token-dispatcher-type alltoall_seq`.'
            )

        if args.moe_adaptive_recompute_activation and args.moe_token_dispatcher_type == 'alltoall_seq':  # nosec B105
            raise AssertionError(
                '`--moe-adaptive-recompute-activation` only support with `--moe-token-dispatcher-type allgather`.'
            )

        if args.moe_allgather_overlap_comm and not args.moe_token_dispatcher_type == 'allgather':  # nosec B105
            raise AssertionError(
        ...
        if args.moe_bmm_mc2:
            if args.moe_model_type != 'megatron_moe' or not args.moe_token_dispatcher_type == 'alltoall':  # nosec B105
        ...
        for action in parser._actions:
            if isinstance(action, argparse._ArgumentGroup):
                ...
                if action.choices is not None and value not in action.choices:
                    action.choices = type(action.choices)(list(action.choices) + [value])
