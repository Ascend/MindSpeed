...
def _add_deepseek_args(parser):
    ...
    # mla arguments
    group.add_argument(
        '--multi-head-latent-attention', action='store_true', default=False, help='Use Multi-head Latent Attention(MLA)'
    )
...
def _add_profile_args(parser):
    ...
    group.add_argument("--profile-with-cpu", action='store_true', default=False, help="Profile with cpu info.")
    group.add_argument("--profile-with-stack", action='store_true', default=False, help="Profile without stack info.")
    group.add_argument("--profile-with-memory", action='store_true', default=False, help="Profile without memory info.")
    group.add_argument("--profile-record-shapes", action='store_true', default=False, help="Profile record shape info.")
    group.add_argument("--profile-save-path", type=str, default='./profile_dir', help="Profile save path.")
    group.add_argument(
        '--profile-ranks',
        nargs='+',
        type=int,
...
def _add_coc_args(parser):
    ...
    group.add_argument("--use-ascend-mc2", action='store_true', help="Use ascend mc2")
    # ascend coc arguments
    group.add_argument("--use-ascend-coc", action='store_true', help="Use ascend coc")
    group.add_argument('--coc-mode', type=int, default=-1, help='coc-mode: 0=original, 1=rewrite, 2=coc default')
    group.add_argument('--coc-parallel-num', type=int, default=1, help='coc parallel num')
    group.add_argument('--coc-fused-kernel', action='store_true', help='use coc fused kernel')
    return parser
...
def _add_moe_args(parser):
    ...
    group.add_argument(
        '--moe-model-type',
        type=str,
        default='megatron_moe',
        choices=['deepspeed_moe', 'megatron_moe'],
        help='moe model type default megatron moe',
    )
    group.add_argument('--expert-interval', type=int, default=1, help='Use experts in every "expert-interval" layers')
    group.add_argument(
        '--moe-train-capacity-factor', type=float, default=1.0, help='The capacity of the MoE expert at training time'
    )
    group.add_argument(
        '--noisy-gate-policy',
        type=str,
        default=None,
        choices=['Jitter', 'RSample', 'None'],
        help="noisy gate policy, valid options are 'Jitter', 'RSample' or 'None'.",
    )
    group.add_argument(
        '--enable-token-rearrange-opt', action='store_true', help="Use this flag to enable token rearrange optimize"
    )
    group.add_argument(
        '--no-use-rts',
        action='store_false',
    ...
    group.add_argument(
        "--moe-no-drop", action='store_true', help="Use no drop policy in moe layer, no tokens will be discarded."
    )
    group.add_argument(
        "--moe-dynamic-padding",
        action='store_true',
    ...
    group.add_argument("--moe-use-sinkhorn", action='store_true', help="Use sinkhorn load balancing in the gate.")

    # megatron mcore moe arguments
    group.add_argument(
        "--moe-tp-extend-ep",
        action='store_true',
        help="use tp group to extend experts parallelisminstead of sharding weight tensor of experts in tp group",
    )
    group.add_argument(
        "--moe-permutation-async-comm", action='store_true', help="overlap moe permutation 3 all gather communications"
    )
    group.add_argument(
    ...
    group.add_argument(
        '--moe-adaptive-recompute-activation-scale',
        type=float,
        default=2.0,
    ...
    group.add_argument(
        '--moe-alltoall-overlap-comm', action='store_true', default=False, help='moe_alltoall_overlap_comm'
    )
    group.add_argument(
        '--moe-allgather-overlap-comm', action='store_true', default=False, help='moe_allgather_overlap_comm'
    )
    group.add_argument(
        "--moe-zero-memory",
        type=str,
    ...
    group.add_argument(
        '--moe-zero-memory-num-layers',
        type=int,
        default=None,
        help='the number of layers using moe-zero-memory level1in each pp stage.',
    )
    group.add_argument('--moe-bmm-mc2', action='store_true', default=False, help='moe_bmm_mc2')
    return parser
...
def _add_cp_args(parser):
    ...
    group.add_argument(
        '--attention-mask-on-cpu', action='store_true', help='store full attention mask on CPU instead of NPU'
    )
    group.add_argument(
    ...
    group.add_argument(
        '--adaptive-cp-dynamic-attn-mask', action='store_true', help='if the attention mask is dynamic across batches'
    )
    group.add_argument(
    ...
    group.add_argument(
        '--adaptive-cp-manually-set-mask-list', action='store_true', help='manually set pre-cooked attention mask list'
    )
    group.add_argument(
        '--context-parallel-kv-cache-policy',
        type=str,
        default=None,
        choices=['full', 'half'],
    ...
    group.add_argument(
        '--context-parallel-cache-interval',
        type=int,
        default=0,
        help='Set the interval of cache layers in cp.Default is 0, means cache K, V in all layers.',
    )
    group.add_argument(
...
def _add_data_args(parser):
    ...
    group.add_argument(
        "--tokenizer-not-use-fast", action='store_false', help="HuggingFace tokenizer not use the fast version."
    )
    return parser
...
def _add_distributed_args(parser):
    ...
    group.add_argument(
        '--use-nanopipe', action='store_true', default=False, help='use nano pipeline parallelism for reduce bubble.'
    )
    group.add_argument(
        '--use-nanopipe-swap',
        action='store_true',
        default=False,
        help='use nano pipeline parallelism with swap for reduce bubble.',
    )
    group.add_argument(
        '--use-pipe-experts', action='store_true', help='Use this flag to enable pipe moe, overlap all2all and expert'
    )
    group.add_argument(
        '--disable-gloo-group',
        action='store_true',
...
def _add_training_args(parser):
    ...
    group.add_argument('--pre-tockens', type=int, default=65536, help='pre-tockens is used by Flash attention')
    group.add_argument('--next-tockens', type=int, default=0, help='next-tockens is used by Flash attention')
    group.add_argument(
        '--shape-order',
        type=str,
        default='SBH',
    ...
    group.add_argument(
        '--sparse-mode', type=int, default=0, help='To improve performance in different modes of attention mask'
    )
    group.add_argument(
    ...
    group.add_argument(
        '--recompute-activation-function', action='store_true', help='Recompute the activation function in MLP layers.'
    )
    group.add_argument(
    ...
    group.add_argument('--recompute-norm', action='store_true', help='Recompute norm in Transformer Layers')
    group.add_argument(
    ...
    group.add_argument('--recompute-in-bubble', action='store_true', help='use bubble to do recompute to reduce memory')
    group.add_argument(
    ...
    group.add_argument(
        '--use-fusion-attn-v2', action='store_true', default=False, help='use fusion_attention ops version 2'
    )
    group.add_argument(
        '--pipe-experts-multi-data',
        type=int,
        default=1,
        help='Use multi data to split the input tensor to implement masking when --use-pipe-experts. The default is 1.',
    )
    group.add_argument(
    ...
    group.add_argument('--use-ema', action='store_true', default=False, help='use ema when training')
    group.add_argument(
    ...
    group.add_argument('--op-cal-tflops', action='store_true', default=False, help='use for cal mfu and hfu')
    group.add_argument(
        '--npu-deterministic', action='store_true', default=False, help='enable deterministic computing for npu'
    )
    group.add_argument(
        '--optimizer-selection',
        type=str,
    ...
    group.add_argument('--ema-decay', type=float, default=0.9999, help='Set ema_decay of fused_ema_adamw optimizer.')
    return parser
...
def _add_network_args(parser):
    ...
    group.add_argument("--add-qkv-bias", action="store_true", default=False, help='Configuration for the qkv bias.')
    group.add_argument("--add-dense-bias", action="store_true", default=False, help='Configuration for the dense bias.')
    group.add_argument("--skip-bias-add", action="store_false", default=True, help='Configuration for the skip bias.')
    group.add_argument("--noop-layers", type=str, help='Specity the noop layers.')
    return parser
...
def _add_automated_pipeline_args(parser):
    ...
    group.add_argument(
        '--automated-pipeline', action='store_true', help='To enable automated pipeline memory saving process'
    )
    group.add_argument(
    ...
    group.add_argument(
        '--save-memory-ratio', type=float, default=0.20, help='To set memory saving rate in automated pipeline'
    )
    ...
    group.add_argument(
        '--optimized-mbs-list', type=str, help='To store the optimized mbs policy of automated pipeline performance'
    )
    group.add_argument('--mbs-idx', type=int, help='To store the index of mbs list')
    group.add_argument(
        '--pp-schedule-list', type=str, help='To store the pipeline schedule policy of automated pipeline performance'
    )
    group.add_argument(
        '--optimized-mbs-mode',
        action='store_false',
    ...
    group.add_argument('--smart-swap', action='store_true', default=False, help='Enable the smart swap feature.')
    return parser
    ...
    def wrapper(args, defaults=None):
        ...
        if args.rope_scaling_type == "yarn":
            if args.rope_scaling_original_max_position_embeddings is None:
                raise AssertionError(
                    'The parameter rope_scaling_original_max_position_embeddings should be set when use yarn.'
                )
        ...
        if args.global_batch_size is None:
            ...
            if args.rank == 0:
                print('Resetting global batch size to {}'.format(args.global_batch_size), flush=True)
        ...
        if args.use_fused_rmsnorm:
            if args.normalization != "RMSNorm":
                raise AssertionError(
                    '--use-fused-rmsnorm must enable with --normalization=RMSNorm, but got normalization={}.'.format(
                        args.normalization
                    )
                )
        ...
        if args.n_shared_experts and args.moe_shared_expert_intermediate_size:
            raise AssertionError(
                '`n_shared_experts` cannot be used with `moe_shared_expert_intermediate_size` together. Please use one of them.'
            )
            ...
            print(
                f'Using shared experts. Convert n_shared_experts to moe_shared_expert_intermediate_size, the moe_shared_expert_intermediate_size is {args.moe_shared_expert_intermediate_size}.'
            )
            ...
            print(
                f'Using shared experts. Convert moe_shared_expert_intermediate_size to n_shared_experts, the n_shared_experts is {args.n_shared_experts}.'
            )
        if args.moe_alltoall_overlap_comm and args.moe_token_dispatcher_type != 'alltoall_seq':  # nosec B105
            raise AssertionError(
                '`--moe-alltoall-overlap-comm` only support with `--moe-token-dispatcher-type alltoall_seq`.'
            )

        if (
            args.moe_adaptive_recompute_activation and args.moe_token_dispatcher_type == 'alltoall_seq'  # nosec B105
        ):
            raise AssertionError(
                '`--moe-adaptive-recompute-activation` only support with `--moe-token-dispatcher-type allgather`.'
            )

        if args.moe_allgather_overlap_comm and args.moe_token_dispatcher_type != 'allgather':  # nosec B105
            raise AssertionError(
        ...
        if args.moe_bmm_mc2:
            if (
                args.moe_model_type != 'megatron_moe' or args.moe_token_dispatcher_type != 'alltoall'  # nosec B105
            ):
        ...
        if args.context_parallel_size > 1 and args.position_embedding_type == 'alibi':
            assert args.context_parallel_algo == 'megatron_cp_algo', "alibi only support megatron_cp_algo"
        if args.context_parallel_size == 1 and args.context_parallel_algo == 'ulysses_cp_algo':
            print(
                "[WARNING] It doesn't make sense to set ulysses_cp_algo when CP=1. Setting it to default: megatron_cp_algo"
            )
            args.context_parallel_algo = 'megatron_cp_algo'
        if args.context_parallel_size > 1 and args.context_parallel_algo == 'ulysses_cp_algo':
            assert args.seq_length % args.context_parallel_size == 0, (
                "sequence length must be divisible by context_parallel_size"
            )
            head, remainder = divmod(
                args.num_attention_heads, args.context_parallel_size * args.tensor_model_parallel_size
            )
            assert head >= 1 and remainder == 0, (
                "num_attention_heads must be divisible by context_parallel_size * tensor_model_parallel_size"
            )
            args.use_flash_attn = True
        if args.context_parallel_size > 1 and args.context_parallel_algo == 'megatron_cp_algo':
            assert args.seq_length % (2 * args.context_parallel_size) == 0, (
                "sequence length must be divisible by 2 * context_parallel_size"
            )
            if args.position_embedding_type == 'alibi':
                assert (args.alibi_fusion_attn_type == 2) and (args.attention_mask_type == 'causal'), (
                    "megatron_cp_algo only support alibi type 2 and attention_mask_type causal"
                )

            assert args.cp_window_size >= 1 and args.cp_window_size < args.context_parallel_size, (
                'cp_window_size should in range [1, context_parallel_size) when using double_ring_attention.'
            )
            n_window, remainder = divmod(args.context_parallel_size, args.cp_window_size)
            assert n_window >= 1 and remainder == 0, (
                'context parallel size must be divisible by cp_window_size when using double ring attention.'
            )
            args.use_flash_attn = True
        if args.context_parallel_size > 1 and args.context_parallel_algo == 'hybrid_cp_algo':
            ...
            assert ring_degree > 1 and remainder == 0, (
                "--ulysses-degree-in-cp must be devisible by --context-parallel-size"
            )
            args.ring_degree = ring_degree

            head, remainder = divmod(
                args.num_attention_heads, args.ulysses_degree_in_cp * args.tensor_model_parallel_size
            )
            assert head >= 1 and remainder == 0, (
                "num_attention_heads must be divisible by ulysse-degree-in-cp * tensor_model_parallel_size in hybrid cp"
            )

            assert args.seq_length % (2 * args.context_parallel_size) == 0, (
                "sequence length must be divisible by 2 * context_parallel_size in hybrid cp"
            )

            assert args.cp_window_size >= 1 and args.cp_window_size < ring_degree, (
                'cp_window_size should be in range [1, ring_degree) when using double ring attention with hybrid context parallelism.'
            )
            n_window, remainder = divmod(ring_degree, args.cp_window_size)
            assert n_window >= 1 and remainder == 0, (
                'ring_degree should be divisible by cp_window_size when using double ring with hybrid context parallelism.'
            )
            args.use_flash_attn = True
        ...
        if args.context_parallel_size > 1 and args.context_parallel_algo == 'hybrid_adaptive_cp_algo':
            assert args.ulysses_degree_in_cp is not None, (
            ...
            assert args.seq_length % args.context_parallel_size == 0, (
                "sequence length must be divisible by context_parallel_size in hybrid cp"
            )
            args.use_flash_attn = True
        if args.context_parallel_size > 1 and args.context_parallel_algo == 'deepseek_v4_cp_algo':
            assert args.attention_mask_type == 'causal', "deepseek_v4_cp_algo only supports causal attention mask type"
            assert args.position_embedding_type != 'alibi', (
                "deepseek_v4_cp_algo does not support alibi position embedding"
            )
            assert args.seq_length % args.context_parallel_size == 0, (
        ...
        if args.recompute_method == "uniform":
            assert not args.recompute_activation_function, (
                'uniform recomputation is not compatible with activation function recomputation '
            )
        ...
        if adaptive_recompute_enable:
            assert args.recompute_granularity is None and args.recompute_method is None, (
                'adaptive selective recompute is not compatible with recompute_granularity and recompute_method. '
            )
            assert not args.recompute_activation_function, (
                'adaptive selective recompute is not compatible with activation function recomputation '
            )
        ...
        if args.adaptive_memory_optimization:
            ...
            assert not adaptive_recompute_enable, (
            ...
            assert not args.recompute_in_bubble, (
                'adaptive memory optimization is not compatible with recompute_in_bubble'
            )
        if args.use_flash_attn:
            assert args.sparse_mode in (0, 2), "Only supports sparse modes 0 and 2"
        ...
        if args.ampipe_degree > 1:
            ...
            if args.context_parallel_size > 1:
                ...
                assert remainder == 0, "sequence length must be divisible by 2 * ampipe_degree * context_parallel_size"
                if args.sequence_parallel:
        ...
        if args.context_parallel_size > 1 and args.reset_attention_mask and args.attention_mask_type == 'causal':
            assert args.context_parallel_algo == 'megatron_cp_algo', (
        ...
        if args.context_parallel_kv_cache_policy:
            ...
            if not args.use_flash_attn:
                raise AssertionError('--context-parallel-kv-cache-policy only support use flash attention.')
        ...
        if args.context_parallel_cache_interval != 0:
            ...
            if args.context_parallel_cache_interval >= args.num_layers:
                raise AssertionError('--context-parallel-cache-interval should be smaller than the number of layers.')
            if args.context_parallel_cache_interval < 0:
                raise AssertionError('--context-parallel-cache-interval cannot be negative number.')

        if args.use_ulysses_allgather_kv:
            if args.context_parallel_size == 1:
                raise AssertionError('context parallel size must larger than 1 when --use-ulysses-allgather-kv is set.')
            ...
            if not args.group_query_attention:
                raise AssertionError('--use-ulysses-allgather-kv needs to enable --group-query-attention.')
    ...
    if parser._actions:
        for action in parser._actions:
            if isinstance(action, argparse._ArgumentGroup):
                add_parser_argument_choices_value(action, argument_name, value)
...
def _add_alibi_args(parser):
    ...
    group.add_argument(
        '--square-alibi-mask', action='store_true', default=False, help='attention mask of alibi is squared'
    )
    group.add_argument('--fill-neg-inf', action='store_true', default=False, help='fill alibi with negative inf')

    group.add_argument('--alibi-fusion-attn-type', type=int, help='alibi pse type, support for 0,2')

    group.add_argument(
        '--alibi-diagonal-opposite', action='store_true', default=False, help='make alibi diagonal opposite'
    )
...
def _add_ndmm_args(parser):
    ...
    group.add_argument(
        '--nd1-dim1-size', type=int, default=1, help='Dim1 of the first nd matmul when use-3d-matmul is True'
    )
    group.add_argument(
        '--nd2-dim1-size', type=int, default=1, help='Dim1 of the second nd matmul when use-3d-matmul is True'
    )
    return parser
...
def _add_2d_tp_args(parser):
    ...
    group.add_argument(
        '--tp-2d', action='store_true', default=False, help='use use-2d-tp to replace megatron-style tensor parallel'
    )
    group.add_argument('--tp-x', type=int, default=1, help='the fist dim tensor parallel size for Linear')
    group.add_argument('--tp-y', type=int, default=1, help='the second dim tensor parallel size for Linear')
    group.add_argument(
...
def _add_hccl_group_buffer_args(parser):
    ...
    group.add_argument(
        '--hccl-group-buffer-adaptive', action='store_true', default=False, help='the hccl buffer for group adaptively'
    )
    group.add_argument(
        '--hccl-ep-group-buffer-adaptive-factor', type=float, default=-1.0, help='the ep group buffer factor'
    )
    return parser
...
def _add_auto_settings_args(parser):
    ...
    group.add_argument("--auto-settings", action="store_true", help="Enable auto settings.")
    group.add_argument(
    ...
    group.add_argument(
        "--auto-settings-ranks", type=int, default=8, help="The world size (# of ranks) for auto settings to search in."
    )
    group.add_argument(
    ...
    group.add_argument("--target-nnodes", type=int, default=1, help="Target search nnodes for auto_settings.")
    group.add_argument(
        "--nnodes",
        type=int,
        default=1,
    ...
    group.add_argument(
        ...
        type=int,
        default=1,
        help="Number of workers per node; supported values: [auto, cpu, gpu, int].Will be passed into torchrun.",
    )
    group.add_argument(
        "--node-rank",
        type=int,
        default=0,
        help="Rank of the node for multi-node distributed training.Will be passed into torchrun.",
    )
    group.add_argument(
        "--auto-settings-type", type=str, default="mixed", help="You should select one of [mixed, white, black]."
    )
