...
def save_checkpoint(
    iteration, model, optimizer, opt_param_scheduler, num_floating_point_operations_so_far, checkpointing_context=None
):
    args = get_args()
    ...
    print_rank_0('saving checkpoint at iteration {:7d} to {} in {} format'.format(iteration, args.save, ckpt_format))
    ...
    if not torch.distributed.is_initialized() or mpu.get_data_modulo_expert_parallel_rank() == 0 or args.use_dist_ckpt:
        optim_sd_kwargs = {}
    ...
    if not torch.distributed.is_initialized() or torch.distributed.get_rank() == 0:
        ...
        def iter_finalize_fn():
            ...
            if args.log_progress and args.async_save:
                append_to_progress_log(args.save, f'Saved async checkpoint\tIteration: {iteration}', barrier=False)
    ...
    if args.use_ema:
        if len(model) == 1:
            ...
            for i in range(len(model)):
                mpu.set_virtual_pipeline_model_parallel_rank(i)
                state_dict['ema%d' % i] = {
                    k.replace('ema.', ''): v for k, v in state_dict['model%d' % i].items() if k.startswith('ema')
                }
                state_dict['model%d' % i] = {
                    k: v for k, v in state_dict['model%d' % i].items() if not k.startswith('ema')
                }
...
def _load_base_checkpoint(
    load_dir, rank0=False, sharded_state_dict=None, exit_on_missing_checkpoint=False, checkpoint_step=None
):
...
def save_checkpoint_ema_wrapper(func):
    ...
    def save_checkpoint_ema(*args, **kwargs):
        model, optimizer, opt_param_scheduler = args[1], args[2], args[3]
