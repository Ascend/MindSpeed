...
class ContextKey:
    ...
    IS_USE_SEND_RECV_LIST = 'is_use_send_recv_list'
...
def is_use_send_recv_list():
    return _IS_USE_SEND_RECV_LIST
...
def _check_config(config_dict):
    ...
    if CK.IS_USE_SEND_RECV_LIST in config_dict.keys() and not isinstance(config_dict[CK.IS_USE_SEND_RECV_LIST], bool):
        raise TypeError(f"The `{CK.IS_USE_SEND_RECV_LIST}` value type must be bool.")
    ...
    valid_keys = [CK.MODEL_CONFIG, CK.IS_USE_SEND_RECV_LIST, CK.MODEL_NAME]
...
def _set_config(config_dict):
    ...
    for k, v in config_dict.items():
        if k == CK.IS_USE_SEND_RECV_LIST:
            global _IS_USE_SEND_RECV_LIST
            _IS_USE_SEND_RECV_LIST = v
...
def _clear_dist_config():
    global _ALL_CONFIG, _RANK_NUMBER_TO_MODEL_NAME, _RANK_NUMBER_TO_MODEL_INDEX, _NUMBER_OF_MODELS, \
        _IS_USE_SEND_RECV_LIST, _ALL_DIST_MODEL_INDEX, _ALL_DIST_MODEL_NAME, _ALL_DIST_MODEL_CONFIG
    _ALL_CONFIG = {}
    _RANK_NUMBER_TO_MODEL_NAME = []
    _RANK_NUMBER_TO_MODEL_INDEX = []
    _NUMBER_OF_MODELS = 0
