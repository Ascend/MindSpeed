...
from mindspeed.auto_tuning.mindspeed_adaptor.mindspeed_settings import MindSpeedSettingsPKL
from mindspeed.auto_tuning.utils.file_utils import check_file_size, restricted_write
from mindspeed.auto_tuning.utils.mem_utils import mem_b_to_mb


def get_settings(args: Namespace, filename: str) -> MindSpeedSettingsPKL:
    ...
    pkl = MindSpeedSettingsPKL()

    pkl.model_cfg = ModelConfig()
    for k in vars(pkl.model_cfg).keys():
        setattr(pkl.model_cfg, k, getattr(args, k, None))
