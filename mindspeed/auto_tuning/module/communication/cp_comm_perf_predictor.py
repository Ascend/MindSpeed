...
from mindspeed.auto_tuning.module.communication.communication_profile import CpProfileTimeInfo
...
class CpCommPerfPredictor(CommPerfPredictor):
    ...
    def get_communication_info_from_profile(
        self, cp_profile_time_info, hcom_info_tag_id, model, cp
    ):
        cp_profile_time_info.total_comm_time += hcom_info_tag_id.total_time_ms
        cp_profile_time_info.wait_comm_time += hcom_info_tag_id.wait_time_ms
        (
            cp_profile_time_info.attn_cp_time,
            cp_profile_time_info.attn_cpbw_time,
        ) = self.get_vectortime_from_profiling(model, cp)
        cp_profile_time_info.overlap_comm_time += hcom_info_tag_id.overlap_time_ms
        cp_profile_time_info.vector_cp_time += hcom_info_tag_id.vector_time_ms
