...
class CpBlock(ChangeBlock):
    ...
    def get_profile(self, origin_profile_data_list):
        change_profile_list = ChangeOperatorList()
    ...
    def get_diff_block(self, change_operator_list, index_id):
        ...
        self.diff_list.bw = self.comp_with_get_diff_list(change_operator_list.list_4.bw, change_operator_list.list_2.bw,
                                                         index_id)
        self.diff_list.fw = self.longest_common_subsequence(self.fw, self.diff_list.fw)
        self.diff_list.re = self.longest_common_subsequence(self.re, self.diff_list.bw)
        self.diff_list.bw = self.longest_common_subsequence(self.bw, self.diff_list.bw)
    ...
    def restart_mat(self, list1, list2, i, j, last_mat):
        m, n = len(list1), len(list2)
        max_miss = 3
        while i < m and j < n:
            if j < n and list1[i].index_name == '' and self.matchv2_v3_operator(list1[i], list2[j]):
