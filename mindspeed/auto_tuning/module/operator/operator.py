...
from mindspeed.auto_tuning.module.operator.operator_elemental import DictCalShape, OperatorLayerTime
from mindspeed.auto_tuning.module.operator.operator_database import DataBase, Operator, OperatorHistory
from mindspeed.auto_tuning.module.operator.operator_shape_analysis import separate_ep_tp_cp
...
class OperatorPerformance(object):
    ...
    def __init__(self, model_config: ModelConfig, working_dir: str):
        ...
        if Settings().waas_enabled and Settings().node_rank == 0:
            from mindspeed.auto_tuning.module.operator.operator_waas import WaasDataBase
            self.waas = WaasDataBase(Settings().waas_ip_addr, Settings().waas_ip_port)
    ...
    def get_history_db(self, operator_note_list):
        ...
        for (index, operator_note) in enumerate(operator_note_list):
            ...
            # 历史数据
            if Settings().waas_enabled and Settings().node_rank == 0 and self.waas.connection:
