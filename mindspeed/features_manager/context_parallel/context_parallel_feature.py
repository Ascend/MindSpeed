...
class ContextParallelFeature(MindSpeedFeature):
    ...
    def validate_args(self, args):
        ...
        if args.context_parallel_size > 1 and args.context_parallel_algo == 'hybrid_cp_algo':
            ...
            if not (ring_degree > 1 and remainder == 0):
                raise AssertionError(
                    "--ulysses-degree-in-cp must be divisible by --context-parallel-size and "
    ...
    def register_patches(self, patch_manager, args):
        ...
        _is_adaptive_cp = _cp_algo in ('adaptive_cp_algo', 'hybrid_adaptive_cp_algo')

        # 第一层：并行组补丁（所有 CP 场景都需注册）
        if _use_cp:
        ...
        # 第二层：核心 CP 后端补丁（仅 TE + Adaptive CP 或 2D TP 扩展需要 MindSpeed CP DPA）
        _need_mindspeed_cp_backend = (_use_cp and _use_te and _is_adaptive_cp) or (
            _cp_expanded_by_2d_tp and _cp_algo == 'megatron_cp_algo'
        )
        if _need_mindspeed_cp_backend:
            from mindspeed.core.context_parallel.adaptor import MindSpeedCPDotProductAttention

            patch_manager.register_patch(
                'megatron.core.extensions.transformer_engine.TEDotProductAttention', MindSpeedCPDotProductAttention
            )
