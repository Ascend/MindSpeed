...
class ContextParallelFeature(MindSpeedFeature):
    ...
    def register_args(self, parser: ArgumentParser):
        ...
        group.add_argument(
            "--megatron-cp-in-bnsd",
            action="store_true",
            help="Megatron CP in BNSD layout.",
        )
    ...
    def validate_args(self, args):
        ...
        use_fused_ring_attention_update = bool(getattr(args, "use_fused_ring_attention_update", False))
        if cp_window_size != 1 or use_cp_send_recv_overlap or use_fused_ring_attention_update:
            if cp_comm_type not in {"p2p", "a2a+p2p"}:
                raise AssertionError(
                    "Ring CP options --cp-window-size, --use-cp-send-recv-overlap and "
                    "--use-fused-ring-attention-update are supported only with "
            ...
            if cp_comm_type == "a2a+p2p":
                ...
                if hierarchy:
                    if len(hierarchy) != 2:
                        raise AssertionError(
                            "a2a+p2p requires two --hierarchical-context-parallel-sizes: [a2a_degree, p2p_degree]."
                        )
                    ...
                    if args.context_parallel_size % ulysses_degree != 0:
                        raise AssertionError("--context-parallel-size must be divisible by --ulysses-degree-in-cp.")
                    ring_cp_size = args.context_parallel_size // ulysses_degree

            if not 1 <= cp_window_size < ring_cp_size:
                raise AssertionError("--cp-window-size must be in [1, ring context-parallel size) for p2p CP.")
            if ring_cp_size % cp_window_size != 0:
                raise AssertionError("ring context-parallel size must be divisible by --cp-window-size for p2p CP.")
    ...
    def register_patches(self, patch_manager, args):
        ...
        if context_parallel_size > 1 and (
            cp_window_size != 1
            or getattr(args, "use_cp_send_recv_overlap", False)
            or getattr(args, "megatron_cp_in_bnsd", False)
            or getattr(args, "use_fused_ring_attention_update", False)
        ):
            from mindspeed.core.context_parallel.tenpu_adaptor import (
