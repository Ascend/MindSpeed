...
class MoETpExtendEpFeature(MindSpeedFeature):
    ...
    def register_args(self, parser: ArgumentParser):
        ...
        group.add_argument(
            ...
            "preserve the initialization used with TP-extended EP disabled",
        )
    ...
    def register_patches(self, patch_manager, args):
        ...
        if hasattr(args, 'moe_token_dispatcher_type') and args.moe_token_dispatcher_type == 'alltoall_seq':  # nosec B105
            patch_manager.register_patch(
                'megatron.core.transformer.moe.router.TopKRouter.routing', routing_tp_extend_ep
            )
            if not args.moe_alltoall_overlap_comm:
                patch_manager.register_patch(
                    'megatron.core.transformer.moe.moe_layer.MoELayer', MindSpeedAlltoAllSEQTptoEpMoELayer
                )
            self._register_initialization_patches(patch_manager)

    @staticmethod
    def _register_initialization_patches(patch_manager):
        from mindspeed.core.transformer.moe.moe_feature.tp_extend_ep.initialization import (
            tp_extend_ep_grouped_mlp_init_wrapper,
        )

        # GMM replaces symbols named GroupedMLP before this patch is applied.
        # This distinct alias still points to the base captured by both the
        # ordinary and overlapping TP-extended expert implementations.
        patch_manager.register_patch(
            "mindspeed.core.transformer.moe.moe_feature.adaptor.MegatronGroupedMLP.__init__",
            tp_extend_ep_grouped_mlp_init_wrapper,
        )
