...
class MoEZeroMemoryFeature(MindSpeedFeature):
    ...
    def pre_validate_args(self, args):
        ...
        if args.moe_zero_memory != "disable" and not (args.moe_alltoall_overlap_comm or args.moe_alltoallseq_overlap_comm):
            raise AssertionError('`--moe-zero-memory` only support `--moe-alltoall-overlap-comm` or `--moe-alltoallseq-overlap-comm` for now.')
