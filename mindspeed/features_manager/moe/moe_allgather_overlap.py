...
class MoEAllGatherOverLapFeature(MindSpeedFeature):
    ...
    def validate_args(self, args):
        ...
        if args.moe_allgather_overlap_comm:
            if getattr(args, 'hybrid_layer_pattern', None) is not None and getattr(
                args, 'recompute_activation_function', False
            ):
                raise AssertionError(
                    '`--moe-allgather-overlap-comm` does not support selective activation recompute '
        ...
        if getattr(args, 'moe_token_dispatcher_type', None) == "allgather":
            if args.moe_allgather_overlap_comm:
                patch_manager.register_patch(
                    'megatron.core.transformer.moe.moe_layer.MoELayer', MindSpeedAllGatherOverlapMoeLayerAdaptor
                )
