...
class DualpipeVFeature(MindSpeedFeature):
    ...
    def validate_args(self, args):
        if args.schedules_method == "dualpipev":
            if getattr(args, 'use_megatron_fsdp', False) or getattr(args, 'use_custom_fsdp', False):
                raise AssertionError("The dualpipev and FSDP are incompatible.")
            if args.overlap_grad_reduce:
                raise AssertionError("The dualpipev and overlap_grad_reduce are incompatible.")
            if not args.untie_embeddings_and_output_weights:
                raise AssertionError("The dualpipev requires untie_embeddings_and_output_weights.")
            if args.swap_attention:
                raise AssertionError("The dualpipev and swap_attention are incompatible.")
            if args.context_parallel_size > 1:
                raise AssertionError("The dualpipev and context_parallel are incompatible.")
            if args.num_layers_per_virtual_pipeline_stage is not None:
                raise AssertionError("The dualpipev and virtual_pipeline are incompatible.")
            if args.pipeline_model_parallel_size == 1:
                raise AssertionError("pipeline_model_parallel_size should be larger than 1 with dualpipev schedules")
            if args.num_layers < args.pipeline_model_parallel_size * 2:
                raise AssertionError('number of layers must be at least 2*pipeline_model_parallel_size in dualpipe')
            ...
            if num_micro_batch < args.pipeline_model_parallel_size * 2 - 1:
                raise AssertionError("num_micro_batch should more than pipeline_model_parallel_size * 2 - 1")
            if args.tp_2d:
                raise AssertionError("The dualpipev and tp_2d are incompatible.")
    ...
    def register_patches(self, patch_manager, args):
        ...
        if args.schedules_method == "dualpipev":
            ...
            patch_manager.register_patch(
                'megatron.core.utils.get_batch_on_this_tp_rank', dualpipev_get_batch_on_this_tp_rank_wrapper
            )
