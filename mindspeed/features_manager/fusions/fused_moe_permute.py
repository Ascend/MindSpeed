...
class FusedMoEPermuteFeature(MindSpeedFeature):
    ...
    def is_need_apply(self, args):
        return self.optimization_level <= args.optimization_level and (
            getattr(args, "moe_permute_fusion", None)
    ...
    def validate_args(self, args: Namespace):
        ...
        if npu_attributes_missing and moe_permute_fusion_enabled:
            warnings.warn(
                ...
                "Please upgrade CANN to 8.3.RC1 and higher, and PTA to 7.2.RC1 and higher "
                "to enable --moe-permute-fusion. "
        ...
        if args.moe_permute_fusion and args.moe_token_dispatcher_type != 'alltoall':  # nosec B105
            raise AssertionError(
                "MindSpeed --moe-permute-fusion currently supports only the alltoall dispatcher "
                "with Megatron 0.17/0.18. The removed alltoall_seq dispatcher and the allgather/flex "
                "dispatchers are not supported."
            )

        router_quantization_padding = getattr(args, 'moe_router_padding_for_quantization', False) or getattr(
            args, 'moe_router_padding_for_fp8', False
        )
        if args.moe_permute_fusion and router_quantization_padding:
            raise AssertionError(
                "MindSpeed --moe-permute-fusion does not yet support Megatron 0.17/0.18 "
    ...
    def register_patches(self, patch_manager, args: Namespace):
        if getattr(args, self.feature_name, None) or getattr(args, "use_fused_moe_token_permute_and_unpermute", None):
            ...
            from mindspeed.core.fusions.fused_moe_permute import permute, unpermute
            ...
            if getattr(args, "moe_token_dispatcher_type", None) == "alltoall":
                from mindspeed.core.fusions.fused_moe_permute import moe_alltoall_token_dispatcher_init_wrapper
