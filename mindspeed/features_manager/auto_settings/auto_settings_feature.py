...
class AutoSettingsFeature(MindSpeedFeature):
    ...
    def is_need_apply(self, args):
        return (
            args.auto_settings
            or os.getenv("OOTB_OPTIMIZER_PARSE_ARGS", "FALSE") == "TRUE"
    ...
    def register_patches(self, patch_manager, args):
        ...
        patch_manager.register_patch('megatron.training.training.pretrain', pretrain_decorator)
        patch_manager.register_patch(
            'megatron.training.training.setup_model_and_optimizer', setup_model_and_optimizer_decorator
        )
        patch_manager.register_patch(
            'megatron.core.tokenizers.utils.build_tokenizer.build_tokenizer', build_tokenizer_wrapper
        )
    ...
    def _add_auto_settings_args(self, parser):
        ...
        group.add_argument(
            "--auto-settings-type", type=str, default="white", help="You should select one of [mixed, white, black]."
        )
        ...
        group.add_argument(
            '--recompute-module-list', type=str, help='To store the recompute policy of automated pipeline'
        )
