...
from megatron.core.pipeline_parallel.schedules import (
    set_current_microbatch, clear_embedding_activation_buffer,
    custom_backward, set_current_microbatch,
    check_first_val_step, finish_embedding_wgrad_compute,
    get_tensor_shapes, recv_forward, recv_backward, send_forward,
    send_backward, send_forward_recv_backward, send_backward_recv_forward)
...
def deallocate_output_tensor(out, deallocate_pipeline_outputs=False):
    ...
    # record tensors original shape before deallocate for checking
    # the scale sense shape consistent with loss shape in the last stage,
    # which might cause some op's bprop error if not.
    out._shape_before_deallocate = out.shape
    out.data = torch.empty((1,), device=out.device, dtype=out.dtype)


# pylint: disable=huawei-too-many-arguments
def forward_step_decorator(forward_step_func,
        data_iterator,
        model,
        num_microbatches,
        input_tensor,
...
def backward_step_decorator(input_tensor, output_tensor, output_tensor_grad, model_type, config):
    argument = get_args()
    if argument.automated_pipeline_perf and not (argument.optimized_mbs_list or argument.pp_schedule_list):
        torch.cuda.synchronize()
        start_time = time.time()
...
def forward_step(
    ...
    # downstream as well.
    # print(f"rank_{torch.distributed.get_rank()}, 1output_tensor:{output_tensor.shape}, 1output_tensor:{output_tensor}, unwrap_output_tensor:{unwrap_output_tensor}", flush=True)
    ...
    *,
    ...
    with no_sync_func():
        for i in range(num_microbatches - 1):
            output_tensor, num_tokens = forward_step_decorator(
                forward_step_func,
                data_iterator,
                model,
                num_microbatches,
            ...
            if not forward_only:
                backward_step_decorator(input_tensor, output_tensor, output_tensor_grad, model_type, config)  # pylint: disable=function-ret
...
def forward_backward_pipelining_with_interleaving(
    *,
        ...
        if (
            ...
            schedule_table.extend(
                [
                    (microbatch_id, model_chunk_id)
                    for model_chunk_id in range(len(model))
                    for microbatch_id in range(  # pylint: disable=complicate-comprehension
    ...
    def forward_step_helper(
        ...
        if input_tensors[model_chunk_id][microbatch_id - offset] is None:
            input_tensors[model_chunk_id][microbatch_id - offset] = torch.tensor(0, dtype=torch.int)
    ...
    def backward_step_helper(virtual_microbatch_id):
        ...
        input_tensor_grad = backward_step_decorator(
    ...
    for k in range(num_warmup_microbatches):
        ...
        if not config.overlap_p2p_comm_warmup_flush:
            if (
                k == (num_warmup_microbatches - 1)  # pylint: disable=too-many-boolean-expressions
