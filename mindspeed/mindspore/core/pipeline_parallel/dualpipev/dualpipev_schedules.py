...
from mindspeed.core.pipeline_parallel.fb_overlap.modules.utils import detach_tensor
from mindspeed.mindspore.core.pipeline_parallel.fb_overlap.modules.utils import dummy_forward_step_func
from mindspeed.core.pipeline_parallel.dualpipev.dualpipev_schedules import (
    set_dualpipe_chunk, is_dualpipev_last_stgae, send_forward, send_backward,
    recv_forward, recv_backward, send_forward_recv_forward,
    send_forward_recv_slave_forward, generate_dualpipev_schedule,
    pretrain_gpt_forward_step_dualpipe, forward_step_no_model_graph,
)
# Types
...
def backward_step_with_model_graph(input_tensor, output_tensor, output_tensor_grad, model_type, config, model_graph=None, model=None):
    ...
    if config.deallocate_pipeline_outputs:
        if model_graph is None:
            ...
            _pynative_executor.check_run(grad_, dummy_forward_step_func, None, None, False, input_tensor[0], create_graph=False)
            _pynative_executor.grad(dummy_forward_step_func, grad_, None, None, input_tensor[0], output_tensor_grad[0])
        else:
...
def forward_step_with_model_graph(
    ...
    if is_dualpipev_last_stgae(model_chunk_id):
        ...
        _pynative_executor.end_graph(dummy_forward_step_func, output_tensor, model_graph.layer_graphs[-1].unperm2_graph[1])
    else:
        _pynative_executor.end_graph(dummy_forward_step_func, output_tensor, input_tensor[0])
            ...
            if args.moe_fb_overlap:
                ...
                if is_dualpipev_last_stgae(slave_chunk_id):
                    ...
                    output_tensor_grad_bwd = backward_step_with_model_graph(
                        input_tensor_bwd, output_tensor_bwd, output_tensor_grad_bwd, model_type, config, model_graph, model[slave_chunk_id]
                    )
        ...
        if args.moe_fb_overlap and not firstFB_no_overlp and not forward_only:
            if not only_bwd:
                ...
                if is_dualpipev_last_stgae(bwd_model_chunk_id):
                    ...
                    input_tensor_grad = backward_step_with_model_graph(
                        input_tensor_bwd, output_tensor_bwd, output_tensor_grad_bwd, model_type, config, model_graph, model[bwd_model_chunk_id]
                    )
