...
def transformer_block_forward(
    ...
    if self.post_process and self.post_layer_norm and self.final_layernorm is not None:
        ...
        _pynative_executor.new_graph(dummy_forward_step_func, detached_hidden_states)

        layer_graphs[-1].unperm2_graph = (layer_graphs[-1].unperm2_graph[0], detached_hidden_states, layer_graphs[-1].unperm2_graph[2])
...
def transformer_block_forward_backward_overlaping(
    ...
    if fwd_block.post_process and fwd_block.post_layer_norm and fwd_block.final_layernorm is not None:
        ...
        detached_hidden_states.retain_grad() # @check 重计算时tensor hook的调用时机
        fwd_layer_graphs[-1].unperm2_graph = (fwd_layer_graphs[-1].unperm2_graph[0], detached_hidden_states, fwd_layer_graphs[-1].unperm2_graph[2])
