...
from mindspeed.mindspore.core.pipeline_parallel.fb_overlap.modules.utils import (
    detach_tensor, run_graph_forward, run_graph_backward, LayerGraph
)
from mindspeed.core.pipeline_parallel.fb_overlap.modules.utils import (is_p2p_comm_needed,
    p2p_comm_helper, P2PCommOutput, P2PCommParams)
from mindspeed.core.pipeline_parallel.fb_overlap.overlap_funcs.fwdbwd import router_forward


def transformer_layer_forward_moe_backward_dense_overlaping_zerc(
        fwd_layer,
        hidden_states,
        attention_mask,
    ...
    with checkpoint_context:
        ...
        hidden_states, attention_forward_vjp = run_graph_forward(attention_forward_func, detached_layer_input,
                                                                 residual1)
        ...
        if recomp_norm:
            ...
            pre_mlp_layernorm_output, pre_mlp_vjp = run_graph_forward(fwd_layer.pre_mlp_layernorm,
                                                                      detached_attention_out)
        # MLP.
        ...
        def alltoall_token_perm1_func(detached_mlp_input, probs_detached):
            ...
            if args.moe_zerc:
                perm1_out, perm1_probs, tokens_per_expert, global_map_info = alltoall_token_perm1(
                    fwd_layer.mlp.token_dispatcher, detached_mlp_input, probs_detached, indices)
                return perm1_out, perm1_probs, global_map_info
            else:
                perm1_out, perm1_probs, tokens_per_expert = alltoall_token_perm1(fwd_layer.mlp.token_dispatcher,
                                                                                 detached_mlp_input, probs_detached,
                                                                                 indices)
        ...
        if args.moe_zerc:
            (perm1_out, perm1_probs, global_map_info), perm1_vjp = run_graph_forward(alltoall_token_perm1_func,
                                                                                     detached_mlp_input,
                                                                                     probs_detached)  # @check tokens_per_expert grad
        else:
    ...
    # bwd for mlp
    detached_mlp_input_grad, residual2_grad = run_graph_backward(bwd_layer_graph.unperm2_graph, bwd_layer_output_grad,
                                                                 keep_grad=True)  # keep for dw
    (detached_attention_out_grad,) = run_graph_backward(bwd_layer_graph.pre_mlp_layernorm_graph,
                                                        detached_mlp_input_grad, keep_graph=True)
    ...
    with checkpoint_context:
        ...
        def mlp_experts(detached_dispached_input, detached_dispached_input_probs):
            ...
            (expert_output, act_ckpt_manager), _ = fwd_layer.mlp.experts(detached_dispached_input, tokens_per_expert,
                                                                         permuted_probs=detached_dispached_input_probs)
            return expert_output

        expert_output, mlp_experts_vjp = run_graph_forward(mlp_experts, detached_dispached_input,
                                                           detached_dispached_input_probs)
        ...
        if args.moe_zero_memory == 'level0':
            ...
            if args.moe_zerc:
                recompute_needed_tensors = [dispached_input, probs, indices,
                                            fwd_layer.mlp.token_dispatcher.select_index,
                                            fwd_layer.mlp.token_dispatcher.nr_token_id_recover]
            else:
                recompute_needed_tensors = [dispached_input, probs, indices,
    ...
    WeightGradStore.start_decouple()
    detached_layer_input_grad, _ = run_graph_backward(bwd_layer_graph.attn_graph, detached_attention_out_grad,
                                                      keep_grad=True)
    WeightGradStore.end_decouple()
    ...
    if next_bwd_layer_graph is not None and getattr(next_bwd_layer_graph, 'is_moe_layer', False):
        (unperm_a2a_graph_grad, detached_shared_expert_output_grad, residual2_grad, probs_grad) = run_graph_backward(
            next_bwd_layer_graph.unperm2_graph, layer_input_grad, keep_graph=True, keep_grad=True)
    ...
    def alltoall_token_unperm2_func(detached_unperm_a2a_out, detached_shared_expert_output, residual2, probs):
        ...
        route_expert_output, unperm2_swap_manager = alltoall_token_unperm2(fwd_layer.mlp.token_dispatcher,
                                                                           detached_unperm_a2a_out, probs)
...
def transformer_layer_forward_moe_backward_moe_overlaping_zerc(
        fwd_layer,
        hidden_states,
        attention_mask,
        bwd_layer_output_grad=None,
    ...
    if bwd_unperm_a2a_handle is None:
        unperm_a2a_graph_grad, shared_expert_output_grad, residual2_grad, probs_grad = run_graph_backward(
            bwd_layer_graph.unperm2_graph, bwd_layer_output_grad, keep_grad=swap_unperm2)        # Async Unperm A2A
    ...
    with checkpoint_context:
        ...
        # perm1_out, perm1_probs, tokens_per_expert = alltoall_token_perm1(fwd_layer.mlp.token_dispatcher,
        #                                                                  detached_mlp_input, probs_detached, indices)
    ...
    bwd_unperm_a2a_handle = None
    mlp_graph_grad, global_map_info_grad = run_graph_backward(bwd_layer_graph.unperm1_graph, unperm1_out_grad)
    ...
    WeightGradStore.start_decouple()
    perm2_graph_grad, detached_dispached_input_probs_grad = run_graph_backward(bwd_layer_graph.grouped_mlp_graph,
                                                                               mlp_graph_grad,
                                                                               keep_grad=True)  # keep for dw
    WeightGradStore.end_decouple()
    ...
    with checkpoint_context:
        ...
        expert_output, mlp_experts_vjp = run_graph_forward(mlp_experts_func, detached_dispached_input,
                                                           detached_dispached_input_probs)
        if args.moe_zero_memory == 'level0':
            dispached_input.untyped_storage().resize_(0)
            if args.moe_zerc:
    ...
    WeightGradStore.start_decouple()
    mlp_input_grad_sharedep, = run_graph_backward(bwd_layer_graph.shared_experts_graph, backward_ag_shared,
                                                  keep_grad=True)  # dw computation
    WeightGradStore.end_decouple()
    ...
    with checkpoint_context:
        ...
        if tp_size > 1 and a2a_hooked_on_attention:
            set_async_alltoall_inputs(
                unperm1_out, fwd_layer.mlp.token_dispatcher.input_splits, fwd_layer.mlp.token_dispatcher.output_splits,
                ep_group
            )
        else:
            _, unperm_a2a_out, unperm_a2a_handle = async_all_to_all(
    ...
    detached_mlp_input_grad = mlp_input_grad_sharedep + detached_mlp_input_grad + detached_mlp_input_router_grad
    (detached_attention_out_grad,) = run_graph_backward(bwd_layer_graph.pre_mlp_layernorm_graph,
                                                        detached_mlp_input_grad, keep_graph=True)
    WeightGradStore.start_decouple()
    detached_layer_input_grad, _ = run_graph_backward(bwd_layer_graph.attn_graph,
                                                      detached_attention_out_grad + residual2_grad, keep_grad=True)
    ...
    if next_bwd_layer_graph is not None and getattr(next_bwd_layer_graph, 'is_moe_layer', False):
        (next_bwd_unperm_a2a_out_grad, next_bwd_shared_expert_output_grad, next_bwd_residual2_grad,
         next_bwd_probs_grad) = run_graph_backward(next_bwd_layer_graph.unperm2_graph, layer_input_grad_sum,
                                                   keep_graph=True, keep_grad=True)
    ...
    with checkpoint_context:
        ...
        output, unperm2_vjp = run_graph_forward(alltoall_token_unperm2_func,
                                                detached_unperm_a2a_out, detached_shared_expert_output, residual2,
                                                probs_detached)
