...
from mindspeed.ops.triton.utils import input_guard, prepare_chunk_indices
...
def merge_16x16_to_64x64_inverse_kernel(
    ...
    if IS_VARLEN:
        ...
        i_t = tl.load(chunk_indices + i_t * 2 + 1)
    ...
    r_off_33 = i_t * BT + 32
    r_off_44 = i_t * BT + 48
    m_diag_11 = (r_off_11 + o_i[:, None] < T) & (o_j[None, :] < BT)
    ...
    b_Ai_11 = tl.load(
        A + (r_off_11 + o_i[:, None]) * H * BT + o_j[None, :],
        mask=m_diag_11,
        other=0.0,
    ).to(tl.float32)
    b_Ai_22 = tl.load(
        A + (r_off_22 + o_i[:, None]) * H * BT + 16 + o_j[None, :],
        mask=m_diag_22,
        other=0.0,
    ).to(tl.float32)
    b_Ai_33 = tl.load(
        A + (r_off_33 + o_i[:, None]) * H * BT + 32 + o_j[None, :],
        mask=m_diag_33,
        other=0.0,
    ).to(tl.float32)
    b_Ai_44 = tl.load(
        A + (r_off_44 + o_i[:, None]) * H * BT + 48 + o_j[None, :],
        mask=m_diag_44,
        other=0.0,
    ).to(tl.float32)
    ...
    b_A_21 = tl.load(
        A + (i_t * BT + 16 + o_i[:, None]) * H * BT + o_j[None, :],
        mask=m_off_21,
        other=0.0,
    ).to(tl.float32)
    b_A_31 = tl.load(
        A + (i_t * BT + 32 + o_i[:, None]) * H * BT + o_j[None, :],
        mask=m_off_31,
        other=0.0,
    ).to(tl.float32)
    b_A_32 = tl.load(
        A + (i_t * BT + 32 + o_i[:, None]) * H * BT + 16 + o_j[None, :],
        mask=m_off_32,
        other=0.0,
    ).to(tl.float32)
    b_A_41 = tl.load(
        A + (i_t * BT + 48 + o_i[:, None]) * H * BT + o_j[None, :],
        mask=m_off_41,
        other=0.0,
    ).to(tl.float32)
    b_A_42 = tl.load(
        A + (i_t * BT + 48 + o_i[:, None]) * H * BT + 16 + o_j[None, :],
        mask=m_off_42,
        other=0.0,
    ).to(tl.float32)
    b_A_43 = tl.load(
        A + (i_t * BT + 48 + o_i[:, None]) * H * BT + 32 + o_j[None, :],
        mask=m_off_43,
        other=0.0,
    ).to(tl.float32)
    ...
    b_Ai_41 = (
        -tl.dot(b_Ai_43, P00, input_precision=DOT_PRECISION)
        - tl.dot(b_Ai_44, P10, input_precision=DOT_PRECISION)
    )
    b_Ai_42 = (
        -tl.dot(b_Ai_43, P01, input_precision=DOT_PRECISION)
        - tl.dot(b_Ai_44, P11, input_precision=DOT_PRECISION)
    )
    ...
    tl.store(
        Ai + (i_t * BT + o_i[:, None]) * H * BT + o_j[None, :],
        b_Ai_11.to(Ai_dt, fp_downcast_rounding="rtne"),
        mask=m_s11,
    )
    tl.store(
        Ai + (i_t * BT + 16 + o_i[:, None]) * H * BT + 16 + o_j[None, :],
        b_Ai_22.to(Ai_dt, fp_downcast_rounding="rtne"),
        mask=m_s22,
    )
    tl.store(
        Ai + (i_t * BT + 32 + o_i[:, None]) * H * BT + 32 + o_j[None, :],
        b_Ai_33.to(Ai_dt, fp_downcast_rounding="rtne"),
        mask=m_s33,
    )
    tl.store(
        Ai + (i_t * BT + 48 + o_i[:, None]) * H * BT + 48 + o_j[None, :],
        b_Ai_44.to(Ai_dt, fp_downcast_rounding="rtne"),
        mask=m_s44,
    )
    tl.store(
        Ai + (i_t * BT + 16 + o_i[:, None]) * H * BT + o_j[None, :],
        b_Ai_21.to(Ai_dt, fp_downcast_rounding="rtne"),
        mask=m_s21,
    )
    tl.store(
        Ai + (i_t * BT + 32 + o_i[:, None]) * H * BT + o_j[None, :],
        b_Ai_31.to(Ai_dt, fp_downcast_rounding="rtne"),
        mask=m_s31,
    )
    tl.store(
        Ai + (i_t * BT + 32 + o_i[:, None]) * H * BT + 16 + o_j[None, :],
        b_Ai_32.to(Ai_dt, fp_downcast_rounding="rtne"),
        mask=m_s32,
    )
    tl.store(
        Ai + (i_t * BT + 48 + o_i[:, None]) * H * BT + o_j[None, :],
        b_Ai_41.to(Ai_dt, fp_downcast_rounding="rtne"),
        mask=m_s41,
    )
    tl.store(
        Ai + (i_t * BT + 48 + o_i[:, None]) * H * BT + 16 + o_j[None, :],
        b_Ai_42.to(Ai_dt, fp_downcast_rounding="rtne"),
        mask=m_s42,
    )
    tl.store(
        Ai + (i_t * BT + 48 + o_i[:, None]) * H * BT + 32 + o_j[None, :],
        b_Ai_43.to(Ai_dt, fp_downcast_rounding="rtne"),
        mask=m_s43,
    )
...
def solve_tril(
    ...
    A must be strictly lower triangular and have shape [B, T, H, BT],
    where BT is one of 16, 32, or 64.
    """
    if A.shape[-1] not in [16, 32, 64]:
        raise ValueError(
            f"A shape BT should be in [16, 32, 64], but current is {A.shape[-1]}"
        )
