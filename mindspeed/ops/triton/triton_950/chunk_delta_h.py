...
def _select_block_k(
    ...
    H: int,
    K: int,
    requested_block_k: int | None,
) -> int:
    if requested_block_k is not None:
        return requested_block_k

    # Empirical heuristic from Ascend 910B3 benchmark data:
    # - block_k=64 is consistently best for short/medium sequences and large H
    # - block_k=128 starts to win on very long sequences with small H
    if K == 128 and T >= 16384 and H <= 4:
        return 128
    return 64
...
@triton.jit(do_not_specialize=['T'])
def chunk_gated_delta_rule_bwd_kernel_dhu_k128_blockdim128(
    q,
    k,
    w,
    g,
    ...
    if TRANSPOSE_STATE:
        b_dh1 = tl.zeros([BV, 128], dtype=tl.float32)
    else:
        b_dh1 = tl.zeros([128, BV], dtype=tl.float32)
    ...
    if USE_FINAL_STATE_GRADIENT:
        if TRANSPOSE_STATE:
            p_dht1 = tl.make_block_ptr(dht, (V, K), (K, 1), (i_v * BV, 0), (BV, 128), (1, 0))
        else:
            p_dht1 = tl.make_block_ptr(dht, (K, V), (V, 1), (0, i_v * BV), (128, BV), (1, 0))
        b_dh1 += tl.load(p_dht1, boundary_check=(0, 1))

    for i_t in range(NT - 1, -1, -1):
        i_t_int64 = i_t.to(tl.int64)
        if TRANSPOSE_STATE:
            p_dh1 = tl.make_block_ptr(dh + i_t_int64*HV*K*V, (V, K), (K, 1), (i_v * BV, 0), (BV, 128), (1, 0))
        else:
            p_dh1 = tl.make_block_ptr(dh + i_t_int64*HV*K*V, (K, V), (V, 1), (0, i_v * BV), (128, BV), (1, 0))
        ...
        p_k = tl.make_block_ptr(k, (T, K), (H*K, 1), (i_t * BT, 0), (BT, 128), (1, 0))
        b_k = tl.load(p_k, boundary_check=(0, 1))
        if USE_GK:
            o_k = tl.arange(0, 128)
            b_gk_last = tl.load(gk + last_idx * HV*K + o_k).to(tl.float32)
        if TRANSPOSE_STATE:
        ...
        p_w = tl.make_block_ptr(w, (K, T), (1, HV*K), (0, i_t * BT), (128, BT), (0, 1))
        p_q = tl.make_block_ptr(q, (K, T), (1, H*K), (0, i_t * BT), (128, BT), (0, 1))
        b_w = tl.load(p_w, boundary_check=(0, 1))
        b_q = tl.load(p_q, boundary_check=(0, 1))
        if USE_G:
            b_dh1 *= bg_last_exp
            b_q = b_q * b_g_exp[None, :].to(b_do.dtype)
        if USE_GK:
            if TRANSPOSE_STATE:
                if USE_EXP2:
                    b_dh1 *= exp2(b_gk_last)[None, :]
                else:
                    b_dh1 *= exp(b_gk_last)[None, :]
            else:
                if USE_EXP2:
                    b_dh1 *= exp2(b_gk_last[:, None])
                else:
    ...
    if USE_INITIAL_STATE:
        if TRANSPOSE_STATE:
            p_dh0 = tl.make_block_ptr(dh0, (V, K), (K, 1), (i_v * BV, 0), (BV, 128), (1, 0))
        else:
            p_dh0 = tl.make_block_ptr(dh0, (K, V), (V, 1), (0, i_v * BV), (128, BV), (1, 0))
...
def chunk_gated_delta_rule_bwd_kernel_dhu_blockdim64(
    ...
    for i_t in range(NT - 1, -1, -1):
        ...
        if USE_G:
            b_dh1 *= bg_last_exp
            b_q = b_q * b_g_exp[None, :].to(b_do.dtype)
            # b_q = b_q.to(tl.float32) * b_g_exp[None, :]
...
def chunk_gated_delta_rule_fwd_h(
    ...
    block_k = _select_block_k(T=T, H=H, K=K, requested_block_k=block_k)
    assert block_k in (64, 128), f"Unsupported block_k={block_k}. Expected one of 64, 128."
    if block_k != 64:
        assert K == 128, "Experimental block_k=128 specialization currently supports K=128 only."
    ...
    if block_k == 128:
        chunk_gated_delta_rule_fwd_kernel_h_k128_blockdim128[grid](
            k=k,
            v=u,
            w=w,
            v_new=v_new,
...
def chunk_gated_delta_rule_bwd_dhu(
    ...
    def grid(meta): return (triton.cdiv(V, meta['BV']), N*HV)
    if K == 128:
        chunk_gated_delta_rule_bwd_kernel_dhu_k128_blockdim128[grid](
            q=q,
            k=k,
            w=w,
            g=g,
