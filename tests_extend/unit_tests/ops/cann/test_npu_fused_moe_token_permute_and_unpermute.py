...
fused_permute = permute_wrapper(permute)
fused_unpermute = unpermute_wrapper(unpermute)
...
class TestNpuFusedPermuteAndUnpermute():
    ...
    def test_permute(self, num_tokens, hidden_size, topk, num_experts, dtype):
        ...
        permuted_tokens_ori, sorted_indices_ori = permute(token_ori, indices_ori, topk=topk)
        permuted_tokens_fused, sorted_indices_fused = fused_permute(token_fused, indices_fused)
    ...
    def test_unpermute(self, num_tokens, hidden_size, topk, num_experts, dtype):
        ...
        unpermuted_tokens_ori = unpermute(
            permuted_tokens_ori, sorted_indices_ori, probs=probs_ori, topk=topk)

        unpermuted_tokens_fused = fused_unpermute(
    ...
    def test_ori_permute_unpermute(self, num_tokens, hidden_size, topk, num_experts, dtype):
        ...
        permuted_tokens, sorted_indices = permute(tokens, indices, topk=topk)
        unpermuted_tokens = unpermute(permuted_tokens, sorted_indices, probs=probs, topk=topk)
    ...
    @pytest.mark.parametrize('num_experts', [4, 128])
    @pytest.mark.parametrize('dtype', [torch.bfloat16])
    def test_npu_fused_permute_unpermute(self, num_tokens, hidden_size, topk, num_experts, dtype):
        tols = TOL_MAPPING.get(dtype)
        tokens = torch.randn(num_tokens, hidden_size).npu().to(dtype)
        indices = torch.randint(0, num_experts, (num_tokens, topk)).npu()
        probs = None
        ...
        permuted_tokens, sorted_indices = fused_permute(tokens, indices, topk=topk)
        unpermuted_tokens = fused_unpermute(permuted_tokens, sorted_indices, probs=probs, topk=topk)
