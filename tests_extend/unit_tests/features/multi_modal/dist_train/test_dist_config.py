...
from mindspeed.multi_modal.dist_train.config import dist_train_config as config
from mindspeed.multi_modal.dist_train.parallel_state import initialize_model_parallel, reset_global_group_and_ranks
from mindspeed.multi_modal.dist_train.communication.dist_ranks_match import clear_model_comm_ranks
from megatron.training import get_args
from unit_tests.common import DistributedTest
from unit_tests.features.multi_modal.dist_train.dist_train_config_utils import get_single_config, make_whole_config
...
class TestDistTrainConfigWithDistributed(DistributedTest):
    ...
    def test_model_name_missing(self):
        ...
        params = (["vit", 0, 2], ["gpt", 1, 2])
        configs = [get_single_config(*param) for param in params]
        configs = {CK.IS_USE_SEND_RECV_LIST: False, CK.MODEL_CONFIG: configs}
        config._clear_dist_config()
        with pytest.raises(KeyError) as exc_info:
            config._set_config(configs)
    ...
    def test_model_config_missing(self):
        """Testing for necessary param not be set in dist-train model configuration."""
        configs = {CK.IS_USE_SEND_RECV_LIST: False, CK.MODEL_NAME: "internvl2"}
        config._clear_dist_config()
        with pytest.raises(KeyError) as exc_info:
            config._set_config(configs)
    ...
    def test_model_config_empty(self):
        ...
        configs = {CK.IS_USE_SEND_RECV_LIST: False, CK.MODEL_CONFIG: [], CK.MODEL_NAME: "internvl2"}
        config._clear_dist_config()
    ...
    def test_model_config_invalid_type(self):
        ...
        configs = {CK.IS_USE_SEND_RECV_LIST: False, CK.MODEL_CONFIG: None, CK.MODEL_NAME: "internvl2"}
        config._clear_dist_config()
        with pytest.raises(TypeError) as exc_info:
            config._set_config(configs)
        assert "`model_config` type must be list" in str(exc_info.value)

    def test_is_use_send_recv_list_invalid_type(self):
        """Testing for abnormal `is_use_send_recv_list` in dist-train model configuration."""
        params = (["vit", 0, 2], ["gpt", 1, 2])
        configs = [get_single_config(*param) for param in params]
        configs = make_whole_config(configs)
        configs["is_use_send_recv_list"] = None
        config._clear_dist_config()
        with pytest.raises(TypeError) as exc_info:
            config._set_config(configs)
        assert "`is_use_send_recv_list` value type must be bool" in str(exc_info.value)
    ...
    def test_get_dist_model_index(self):
        ...
        config_ = self.set_normal_whole_config()
        model_indexes = [cfg.get(CK.MODEL_INDEX) for cfg in config_[CK.MODEL_CONFIG] for _ in range(cfg[CK.WORLD_SIZE])]
        rank = torch.distributed.get_rank()
