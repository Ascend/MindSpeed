...
import mindspeed.multi_modal.dist_train.communication.dist_communication as comm
from mindspeed.multi_modal.dist_train.config.dist_train_config import _set_config, _clear_dist_config
...
class TestDistCommunicationWithDistributed(DistributedTest):
    ...
    def test_dist_model_send_recv_tensor(self, src_dst_ranks: Tuple[List, List]):
        ...
        if rank in src:
            idx = src.index(rank)
            for _ in range(idx):
                get_manual_tensor(idx)  # align tensor
    ...
    def _test_generate_send_recv_ops(self, is_fwd_only_pair, ans_ops):
        ...
        rank = torch.distributed.get_rank()
        ops = comm.generate_send_recv_ops(rank)
