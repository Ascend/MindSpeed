...
class TestQos:
    ...
    def test_resolve_parallel_type(self):
        ...
        assert resolve_parallel_type('hcp') == 'cp'
        assert resolve_parallel_type('intra_dp_cp') == 'dp-cp'
        assert resolve_parallel_type('inter_dp_cp') == 'dp-cp'
        ...
        assert resolve_parallel_type('intra_ep_dp') == 'ep-dp'
        assert resolve_parallel_type('inter_ep_dp') == 'ep-dp'
        assert resolve_parallel_type('intra_dist_opt_instance') == 'dp'
...
class TestQosFeature:
    def test_register_args(self):
        parser = ArgumentParser()
        QosFeature().register_args(parser)

        defaults = parser.parse_args([])
        assert defaults.aiqos is False
        assert defaults.aiqos_mode == 'auto'
        assert defaults.aiqos_schedule is None
        assert defaults.aiqos_enable_roce is False

        schedule = '{tp:high,pp:middle,dp:low}'
        args = parser.parse_args(
            ['--aiqos', '--aiqos-mode', 'manual', '--aiqos-schedule', schedule, '--aiqos-enable-roce']
        )
        assert args.aiqos is True
        assert args.aiqos_mode == 'manual'
        assert args.aiqos_schedule == schedule
        assert args.aiqos_enable_roce is True

    def test_register_patch_uses_megatron_018_option_builder(self):
        patch_manager = MagicMock()
        args = SimpleNamespace(aiqos=True, aiqos_mode='auto', aiqos_schedule=None)

        QosFeature().register_patches(patch_manager, args)

        patch_manager.register_patch.assert_called_once_with(
            'megatron.core.parallel_state.get_nccl_options', get_nccl_options_qos_wrapper
        )

    def test_manual_mode_requires_schedule(self):
        args = SimpleNamespace(aiqos=True, aiqos_mode='manual', aiqos_schedule=None)

        with pytest.raises(ValueError, match='requires --aiqos-schedule'):
            QosFeature().register_patches(MagicMock(), args)
