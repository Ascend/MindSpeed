...
from mindspeed.te.pytorch.attention.dot_product_attention import (
    kvallgather_context_parallel as kvallgather_cp,
)
...
def _diff_cu_seqlens(cu_seqlens):
    return [cu_seqlens[0]] + [
        cu_seqlens[idx] - cu_seqlens[idx - 1] for idx in range(1, len(cu_seqlens))
    ]
...
def test_thd_load_balanced_cp_metadata(cp_size, cu_seqlens):
    ...
    rank_workloads = []
    for rank in range(cp_size):
        metadata = get_thd_load_balanced_cp_metadata(
        ...
        rank_workloads.append(
            sum(q_len * kv_len for q_len, kv_len in zip(q_lens, kv_lens))
        )
...
def test_thd_metadata_tensor_identity_cache_avoids_reconversion():
    ...
    cache_info = tensor_metadata_cache.cache_info()  # pylint: disable=no-value-for-parameter
    cached_metadata = kvallgather_cp._get_thd_load_balanced_cp_metadata_cached(
        cu_seqlens, 2, 0, torch.device("cpu")
    )
    updated_cache_info = tensor_metadata_cache.cache_info()  # pylint: disable=no-value-for-parameter
...
def test_thd_metadata_tensor_mutation_invalidates_identity_cache():
    ...
    cu_seqlens[-1] = 80
    updated_metadata = kvallgather_cp._get_thd_load_balanced_cp_metadata_cached(
        cu_seqlens, 2, 0, torch.device("cpu")
    )
...
class TestKVAllGatherContextParallelTHD(DistributedTest):
    ...
    def test_forward_and_backward_match_non_cp_reference(
        ...
        assert torch.allclose(
            out, out_ref.detach().index_select(0, local_index), **tolerances
        )
        assert torch.allclose(
            q.grad, q_ref.grad.index_select(0, local_index), **tolerances
        )
        assert torch.allclose(
            k.grad, k_ref.grad.index_select(0, local_index), **tolerances
        )
        assert torch.allclose(
            v.grad, v_ref.grad.index_select(0, local_index), **tolerances
        )
