...
run_st() {
    ...
    cd "${workspace}"
    cp -rf "${MEGATRON_DIR}" ./
    # Prepend the branch-checked-out Megatron to PYTHONPATH so subprocesses
    # (e.g. verl Ray workers) resolve it instead of the image default (core_v0.12.1).
    ...
    # ============================================
    # ST 3: verl – skip on branches without args_utils.py, on dev, or legacy
    # ============================================
    if [ -f "${code_dir}/mindspeed/args_utils.py" ] && \
       ...
       [[ "$branch" != "dev" ]]; then
        ...
        echo "Skipping verl ST (args_utils.py absent, dev, or legacy branch)"
    fi
}
