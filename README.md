...
- [May 21, 2025]: 🚀 MindSpeed Core 支持Mcore 0.15.3版本。
...
-  注意❗：当前 MindSpeed core_r0.15.3 分支正在持续适配 Megatron core v0.15.3 中，**仅"Megatron特性支持"部分已完成适配验证**，其余特性（并行策略特性、内存优化特性、亲和计算特性、通信优化特性、Mcore MoE特性、关键场景特性、多模态特性等）尚未完成全面测试，使用时请注意。

-  注意❗：除"Megatron特性支持"外，以下各表中的 ✅ 仅表示该特性在历史版本中已商用发布，**不代表已在 core_r0.15.3 上完成适配验证**。
