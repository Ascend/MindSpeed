...
The following figure shows the PP communication group profiling collected using the PP8 TP2 EP32 DualPipeV strategy on the DeepSeek-V3-671B model.
...
1. Add `--schedules-method dualpipev` to the launch script to enable the DualPipeV pipeline arrangement.

2. Additionally add `--moe-fb-overlap` to the launch script to enable MoE cross-microbatch forward and backward communication overlap.

3. Additionally add `--dualpipev-dw-detach` to the launch script to enable dw detachment in the cooldown stage.

4. When using DualPipeV, set the number of model layers to a multiple of `PP*2`. Also, the number of micro batches per PP group should be set to at least `PP*2`. It is incompatible with other pipeline features such as VPP. It is also currently incompatible with features such as long sequence parallelism, asynchronous DDP, swap-attention, and tp_2d.
...
> [!NOTE]
>
> Currently, `use_custom_fsdp` and `dualpipev` are incompatible.
