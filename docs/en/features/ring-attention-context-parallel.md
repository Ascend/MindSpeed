...
The Ring Attention long-sequence parallelism scheme is supported to address the sequence dimension scaling problem. For details, see the paper [Ring Attention with Blockwise Transformers for Near-Infinite Context](<https://arxiv.org/pdf/2310.01889>).
...
The Ring Attention long-sequence parallel scheme is applicable to the following typical scenarios:
...
| Important Parameter | Parameter Description | Required | Default Value |
|---------------------|-----------------------|----------|---------------|
| `--context-parallel-size [int]` | Sets the number of parallel processes for CP. Configure according to user requirements. | No | 1 |
| `--seq-length [int]` | The length of the input sequence. | Yes | / |
| `--use-cp-send-recv-overlap` | Recommended to enable. When enabled, supports send/receive overlap functionality. | No | True |
| `--attention-mask-type` | Sets the mask computation type. The default is `causal` (causal mask). Setting to `general` enables full attention computation. | No | causal |
| `--context-parallel-algo` | Long-sequence parallel algorithm options:<ul><li><code>ulysses_cp_algo</code>: Enables Ulysses long-sequence parallel</li><li><code>hybrid_cp_algo</code>: Enables Hybrid long-sequence parallel</li><li><b><code>megatron_cp_algo</code></b>: Enables Ring Attention long-sequence parallel</li></ul> | No | megatron_cp_algo |
| `--megatron-cp-in-bnsd` | When enabled, FlashAttention uses the BNSD computation format. | No | True |
| `--cp-window-size [int]` | Controls the inner window size of the two-level Ring Attention. When set to `1`, the original Ring Attention algorithm is used. When set to a value greater than `1`, the Double Ring Attention algorithm is used to optimize performance. Requires that `cp_size` be divisible by this parameter. | No | 1 |
