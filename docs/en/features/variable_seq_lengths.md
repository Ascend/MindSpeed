...
> - `--moe-token-dispatcher-type alltoall_seq` and `--moe-token-dispatcher-type allgather` are not supported.

To enable PP support for dynamic shapes, add the following parameters to the training script"

```bash
# Enable pipeline parallelism, PP >= 2
--pipeline-model-parallel-size ${PP} \
# Enable PP support for dynamic shapes
