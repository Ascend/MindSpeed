...
The Double Ring Attention algorithm is supported, further accelerating the original Ring Attention implementation. For algorithm details, refer to [LoongTrain: Efficient Training of Long-Sequence LLMs with Head-Context Parallelism](<https://arxiv.org/pdf/2406.18485>).
...
Training scenarios where Ring Attention is already enabled. For how to enable Ring Attention, refer to [Ring Attention](ring-attention-context-parallel.md).
...
| Parameter                | Description         | Required (Yes/No) | Default Value |
|------------------------|-----------------|---------|----------|
| `--cp-window-size [int]` | Controls the inner window size of the two-level Ring Attention. When set to `1`, the original algorithm is used. When set to a value greater than `1`, the Double Ring Attention algorithm is used to optimize the performance of the original Ring Attention. Requires that `--context-parallel-size` be divisible by this parameter. | No | 1 |
...
> Ensure that `--cp-window-size` is less than `--context-parallel-size`.
