...
![virtual-optimizer](../figures/virtual-optimizer_en.png)
...
The allocated host virtual memory cannot perform on-the-fly computation (as there is no hardware on-the-fly computation unit).

## Usage

When you need to swap the memory of the optimizer section, there are several scenarios:

- If you want to swap all first-order and second-order momentums, you can specify `--virtual-optimizer all`.
- If you want each PP stage to swap the same amount of memory (for example, you want each stage to swap 2 GB of memory), you can specify `--virtual-optimizer 2.0`.
- If you want each PP stage to swap different amounts of memory (assuming there are four PP stages, and you want to swap 6, 5, 4, and 3 GB of memory respectively), you can specify `--virtual-optimizer 6 5 4 3`.
