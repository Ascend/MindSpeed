...
2. For GQA models with a single head per rank, add an AllGather KV + All2All Q approach on top of Ulysses Attention long-sequence parallelism. This reduces repeat operations and the overhead of non-contiguous memory operations such as transpose, thereby improving training performance.
...
Enabling KV cache in Ulysses Attention will reduce memory usage when Repeat is applied in Repeat-All2All, but it will cause performance degradation. In the AllGather case, memory usage will also be reduced, but performance may drop compared to the AllGather scheme without caching.
...
4. When both `--context-parallel-kv-cache-policy` and `--context-parallel-algo ulysses_cp_algo` are enabled, the KV repeat condition must be satisfied (i.e., the number of KV heads must be less than the CP world size, and the number of query heads must be divisible by the number of KV heads). Otherwise, the feature is not supported. KV repeat is typically satisfied automatically when using GQA.
