...
During MoE model training, expert load exhibits an imbalance phenomenon, especially in the pre-training phase. The number of tokens processed by experts on different devices varies significantly, and the load changes drastically, causing fast-slow device issues that degrade throughput.
...
where $ELP_t$ is the expert load prediction at step $t$, $EL_t$ is the collected expert load data at step $t$, and $θ$ is the EMA weight, typically ranging from 0.9 to 0.999. A larger $θ$ means the load prediction focuses more on historical data, with less influence from the current load data. In this solution, θ=0.9 is used. Compared to larger θ values, the current load data is given higher weight, enabling better capture of current load characteristics. As a result, the expert migration based on this approach adapts more effectively to changing loads.
...
Supports `reuse_fp32_param`, `use_distributed_optimizer`, and `tp_extend_ep`.
