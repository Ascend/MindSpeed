...
In the default FP8 training architecture, the original weights loaded by the model are in `BF16` format.
During forward or backward propagation, the framework calls `npu_dynamic_mx_quant` to convert `BF16` to `FP8` for matrix multiplication.

The problem lies here:

- After conversion, new FP8 weights are generated (stored in the Reuse Cache pool).
...
## 2. Initial Ideas and Final Implementation

| Core Pain Points and Initial Ideas | Code Implementation | Meaning |
| --- | --- | --- |
...
| **Recomputation activating/backward recomputation** | Not involved | Recomputation is currently incompatible with BF16 weight release. |
...
We conducted rigorous probe validation and loss alignment under different model architectures and sharding strategies, and the device memory benefit perfectly closed the loop with the theoretical derivation. From the perspective of the **model's global static device memory (Model States)**, the theoretical device memory reduction ratio of this solution is approximately **10.5% (2N/19N)**.
...
- **Configuration**: Dense 6 layers, TP=4, total 2.93B parameters. The theoretical BF16 device memory benefit is 2.93*2, approximately 5.8GB.
- **Measured data**:
...
- **Configuration**: MoE model, 2 layers, EP=2. Each card is responsible for 8 experts. The theoretical BF16 device memory benefit is 0.75 * 2, approximately 1.5 GB.
- **Measured data**:
