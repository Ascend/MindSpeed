...
1. Refer to the [MindSpeed Quick Start](https://www.hiascend.com/en/developer/software/mindspeed/download) to prepare the environment. For more details, see [MindSpeed Installation Guide](install_guide.md).
...
> [!NOTE]
>
> MindSpeed Core supports<term>Ascend 950 products</term>, <term>Atlas A3 training products</term>, and <term>Atlas A2 training products</term>, and requires a single NPU with on-chip memory of 64GB or higher.
>
> n the current example script, `NPUS_PER_NODE=8` indicates that 8 NPUs are required. If the actual configuration is lower than this, OOM issues may occur.
...
1. Download the [Tokenizer](https://huggingface.co/Xenova/gpt-3.5-turbo/tree/main).
    ...
    >[!NOTE]
    >
    > You need to configure a proxy on your own to access or download resources. If you are unable to successfully download resources from the Hugging Face community, it is recommended to download them from ModelScope instead, but be sure to verify the correctness and security of the files to be downloaded.
...
4. Run the script to start pretraining.
    ...
    > [!NOTE]
    >
    > Some parameters in the sample `train_distributed.sh` script, such as `--hidden-size` and `--num-layers`, need to be adapted to the actual scenario to avoid OOM and similar issues.
