...
## 使用约束
1. 当前仅支持`--moe-token-dispatcher-type=alltoall`, 暂不支持`allgather` Dispatcher。
2. 不建议同时使用`--swap-attention`，开启后性能劣化。
3. 当前暂不支持Megatron MoE Token Drop&Pad模式，仅支持Dropless。
4. 当前依赖GroupedMatmul, 确保打开`--moe-grouped-gemm`。
5. 当前仅支持`--moe-zero-memory=level0`，且不支持`moe-zero-memory-num-layers`配置。
6. 当前暂不支持异步DP通信掩盖，需关闭`--overlap-grad-reduce`。
7. 当前仅支持Mcore Models，不能打开`--use_legacy_models`。
8. 与下列特性冲突，不能同时使用：
