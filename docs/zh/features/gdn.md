...
MindSpeed 基于 Megatron Core 的 GatedDeltaNet 模块，面向昇腾 NPU 训练场景补充了以下适配。当前 MindSpeed GDN 实现会在 CP、Packed Sequence/EOD pack、2D TP 相关场景或开启 `--use-naive-l2norm` 时替换 Megatron Core 原生实现：

- **GDN 特性接入**：支持通过 `--experimental-attention-variant gated_delta_net` 使用 Megatron GDN 线性注意力变体。
- **NPU 核心算子接入**：安装 `fla_npu` 时优先使用 NPU Gated Delta Rule 和 NPU causal conv1d 算子路径。
- **Triton/Torch 回退实现**：未安装 `fla_npu` 时，Gated Delta Rule 回退到 MindSpeed Triton 实现；开启确定性模式时使用 `torch_chunk_gated_delta_rule`。
- **因果卷积回退**：未安装 `fla_npu` 或开启确定性模式时，GDN 内部使用 PyTorch Conv1d 回退路径。
...
GDN 的 CP 路径默认采用 All-to-All 头序转换：计算前将本地序列切分转换为完整序列上的部分 head，计算后再转换回序列切分布局。当前 packed 模式需要通过 `megatron_cp_algo` 开启 CP；该配置项用于通过参数校验并生成 Megatron CP 的 packed 序列布局，GDN 层内部不走 full Attention 的 Ring Attention 计算。
...
- `seq-length` 能被 `2 * context-parallel-size` 整除。
...
- 使用 CP 时，packed 后每个子序列长度需要能被 `2 * context-parallel-size` 整除。
...
该路径主要用于确定性复现，性能通常低于 NPU/Triton 路径，并且不支持 Packed Sequence。若使用 MindSpeed 的全局 NPU 确定性开关，可参考 `docs/zh/features/npu_deterministic.md` 中的 `--npu-deterministic` 说明。
...
1. **依赖限制**：GDN 初始化依赖 `flash-linear-attention`（FLA）库；使用 NPU 高性能路径需要安装并正确编译 `fla_npu` 相关算子，未安装 `fla_npu` 时会回退到 MindSpeed Triton Gated Delta Rule 和 PyTorch Conv1d 路径。
...
5. **CP 算法限制**：GDN 默认使用 All-to-All 头序转换路径，当前 packed 模式只支持配置 `megatron_cp_algo`；该配置项仅用于开启 CP 并生成 Megatron CP packed 布局，GDN 不走 full Attention 的 CP 计算路径。
