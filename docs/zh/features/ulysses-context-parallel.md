...
随着生成时AI与科研模型的飞速发展，长序列训练已成为关键驱动力。从会话式AI、长文档摘要到视频生成，系统都需要在广阔的空间和时间维度上实现对长上下文的有效推理。特别是面对章节级乃至书籍级的海量文本（数万至数十万字），传统会话式AI与摘要任务正面临严峻挑战。然而，传统的数据、张量和流水线等并行处理方法在处理长序列时面临显著限制。这些方法往往无法有效扩展以适应序列维度的增加，从而影响了系统整体的性能和效率。
...
> [!NOTE]
>
> - 对于Group Query Attention模型，需要确保num_attention_heads能被num_query_groups能被整除。
