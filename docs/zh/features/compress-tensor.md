...
通过昇腾无损压缩算子压缩激活值占用的内存。可使用下面的激活压缩入口。
...
- 特性二：
    `--compress-activation` 表示压缩哪些层产生的激活值，可配置为`1-3`或`1,2,3`，表示压缩1/2/3层产生的激活值，可配置为`1-3,5-8,10`表示压缩1/2/3/5/6/7/8/10层产生的激活值，也可配置为0表示压缩所有层，默认值为空字符串（不启用）。

## 使用效果

使用激活值压缩特性会降低峰值内存。在如llama，qwen等典型模型下，开启特性一/二平均内存收益在3%/7%。如：qwen3-8b模型在序列长度为8192，并行配置为PP4TP2CP1，开启特性二的情况下获得内存收益6.2%。

## 使用约束

- 激活值压缩需配套 PTA 支持。

- 特性二为特性一的迭代版本，训练时选择其中一种。
