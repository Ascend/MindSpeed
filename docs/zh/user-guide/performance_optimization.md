...
- `--cache-get-batch-config`：缓存 get_batch 首次生成的完整 TransformerConfig，减少重复配置开销，默认关闭，详见 [get_batch 配置缓存](../features/cache-get-batch-config.md)。
- `--async-log-allreduce`：开启日志异步规约，默认关闭。
